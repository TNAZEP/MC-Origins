...
public abstract class Pathfinder {
	...
	public PathEntity createPath(double minX, double minY, double minZ, float width, float height, double var2, double var4, double var6, float var8) {
		this.path.clearPath();
		this.pointMap.clearMap();
		PathPoint var9 = this.openPoint(MathHelper.floor_double(minX), MathHelper.floor_double(minY), MathHelper.floor_double(minZ));
		PathPoint var10 = this.openPoint(MathHelper.floor_double(var2 - (double)(width / 2.0F)), MathHelper.floor_double(var4), MathHelper.floor_double(var6 - (double)(width / 2.0F)));
		PathPoint var11 = new PathPoint(MathHelper.floor_float(width + 1.0F), MathHelper.floor_float(height + 1.0F), MathHelper.floor_float(width + 1.0F));
		PathEntity var12 = this.addToPath(var9, var10, var11, var8);
		return var12;
	}

	private PathEntity addToPath(PathPoint var2, PathPoint var3, PathPoint var4, float var5) {
		...
		while(!this.path.isPathEmpty()) {
			...
			var7.isFirst = true;
			int var8 = this.findPathOptions(var7, var4, var3, var5);
	...
	private int findPathOptions(PathPoint var2, PathPoint var3, PathPoint var4, float var5) {
		int var6 = 0;
		byte var7 = 0;
		if(this.getVerticalOffset(var2.xCoord, var2.yCoord + 1, var2.zCoord, var3) == 1) {
			var7 = 1;
		}

		PathPoint var8 = this.getSafePoint(var2.xCoord, var2.yCoord, var2.zCoord + 1, var3, var7);
		PathPoint var9 = this.getSafePoint(var2.xCoord - 1, var2.yCoord, var2.zCoord, var3, var7);
		PathPoint var10 = this.getSafePoint(var2.xCoord + 1, var2.yCoord, var2.zCoord, var3, var7);
		PathPoint var11 = this.getSafePoint(var2.xCoord, var2.yCoord, var2.zCoord - 1, var3, var7);
	...
	private PathPoint getSafePoint(int var2, int var3, int var4, PathPoint var5, int var6) {
		PathPoint var7 = null;
		if(this.getVerticalOffset(var2, var3, var4, var5) == 1) {
			var7 = this.openPoint(var2, var3, var4);
		}

		if(var7 == null && var6 > 0 && this.getVerticalOffset(var2, var3 + var6, var4, var5) == 1) {
		...
		if(var7 != null) {
			...
			while(var3 > 0) {
				var9 = this.getVerticalOffset(var2, var3 - 1, var4, var5);
				if(var9 != 1) {
					break;
				}
	...
	protected abstract int getVerticalOffset(int x, int y, int z, PathPoint size);
