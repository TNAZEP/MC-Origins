...
public class Uniform extends AbstractUniform implements AutoCloseable {
   ...
   public Uniform(String var1, int var2, int var3, Shader var4) {
      ...
      this.parent = â˜ƒ;
      if (â˜ƒ <= 3) {
         this.intValues = MemoryUtil.memAllocInt(â˜ƒ);
         this.floatValues = null;
      } else {
         this.intValues = null;
         this.floatValues = MemoryUtil.memAllocFloat(â˜ƒ);
      }
   ...
   public static int getTypeFromString(String var0) {
      ...
      if ("int".equals(â˜ƒ)) {
         â˜ƒ = 0;
      } else if ("float".equals(â˜ƒ)) {
         â˜ƒ = 4;
      } else if (â˜ƒ.startsWith("matrix")) {
         if (â˜ƒ.endsWith("2x2")) {
            â˜ƒ = 8;
         } else if (â˜ƒ.endsWith("3x3")) {
            â˜ƒ = 9;
         } else if (â˜ƒ.endsWith("4x4")) {
            â˜ƒ = 10;
         }
      }
   ...
   public final void set(float var1, float var2) {
      this.floatValues.position(0);
      this.floatValues.put(0, â˜ƒ);
      this.floatValues.put(1, â˜ƒ);
      this.markDirty();
   }

   public final void set(int var1, float var2) {
      this.floatValues.position(0);
      this.floatValues.put(â˜ƒ, â˜ƒ);
      this.markDirty();
   }
   ...
   public final void set(Vector3f var1) {
      this.floatValues.position(0);
      this.floatValues.put(0, â˜ƒ.x());
      this.floatValues.put(1, â˜ƒ.y());
      this.floatValues.put(2, â˜ƒ.z());
      this.markDirty();
   }
   ...
   public final void setSafe(int var1, int var2, int var3, int var4) {
      ...
      if (this.type >= 0) {
         this.intValues.put(0, â˜ƒ);
      }

      if (this.type >= 1) {
         this.intValues.put(1, â˜ƒ);
      }

      if (this.type >= 2) {
         this.intValues.put(2, â˜ƒ);
      }

      if (this.type >= 3) {
         this.intValues.put(3, â˜ƒ);
      }
   ...
   @Override
   public final void set(float[] var1) {
      if (â˜ƒ.length < this.count) {
         LOGGER.warn("Uniform.set called with a too-small value array (expected {}, got {}). Ignoring.", this.count, â˜ƒ.length);
      } else {
         this.floatValues.position(0);
         this.floatValues.put(â˜ƒ);
         this.floatValues.position(0);
   ...
   public final void setMat2x3(float var1, float var2, float var3, float var4, float var5, float var6) {
      ...
      this.floatValues.put(2, â˜ƒ);
      this.floatValues.put(3, â˜ƒ);
      this.floatValues.put(4, â˜ƒ);
      this.floatValues.put(5, â˜ƒ);
      this.markDirty();
   }
   ...
   public final void setMat2x4(float var1, float var2, float var3, float var4, float var5, float var6, float var7, float var8) {
      ...
      this.floatValues.put(4, â˜ƒ);
      this.floatValues.put(5, â˜ƒ);
      this.floatValues.put(6, â˜ƒ);
      this.floatValues.put(7, â˜ƒ);
      this.markDirty();
   }
   ...
   public final void setMat3x4(
      ...
      this.floatValues.put(7, â˜ƒ);
      this.floatValues.put(8, â˜ƒ);
      this.floatValues.put(9, â˜ƒ);
      this.floatValues.put(10, â˜ƒ);
      this.floatValues.put(11, â˜ƒ);
      this.markDirty();
   }
   ...
   public final void setMat4x4(
      ...
      this.floatValues.put(10, â˜ƒ);
      this.floatValues.put(11, â˜ƒ);
      this.floatValues.put(12, â˜ƒ);
      this.floatValues.put(13, â˜ƒ);
      this.floatValues.put(14, â˜ƒ);
      this.floatValues.put(15, â˜ƒ);
      this.markDirty();
   }
