...
public class InputConstants {
   ...
   public static InputConstants.Key getKey(int var0, int var1) {
      return â˜ƒ == -1 ? InputConstants.Type.SCANCODE.getOrCreate(â˜ƒ) : InputConstants.Type.KEYSYM.getOrCreate(â˜ƒ);
   }

   public static InputConstants.Key getKey(String var0) {
      if (InputConstants.Key.NAME_MAP.containsKey(â˜ƒ)) {
         return (InputConstants.Key)InputConstants.Key.NAME_MAP.get(â˜ƒ);
      } else {
         for(InputConstants.Type â˜ƒ : InputConstants.Type.values()) {
            if (â˜ƒ.startsWith(â˜ƒ.defaultPrefix)) {
               String â˜ƒx = â˜ƒ.substring(â˜ƒ.defaultPrefix.length() + 1);
               return â˜ƒ.getOrCreate(Integer.parseInt(â˜ƒx));
            }
         }

         throw new IllegalArgumentException("Unknown key name: " + â˜ƒ);
      }
   }

   public static boolean isKeyDown(long var0, int var2) {
      return GLFW.glfwGetKey(â˜ƒ, â˜ƒ) == 1;
   }

   public static void setupKeyboardCallbacks(long var0, GLFWKeyCallbackI var2, GLFWCharModsCallbackI var3) {
      GLFW.glfwSetKeyCallback(â˜ƒ, â˜ƒ);
      GLFW.glfwSetCharModsCallback(â˜ƒ, â˜ƒ);
   }
   ...
   public static void setupMouseCallbacks(
      ...
      GLFW.glfwSetCursorPosCallback(â˜ƒ, â˜ƒ);
      GLFW.glfwSetMouseButtonCallback(â˜ƒ, â˜ƒ);
      GLFW.glfwSetScrollCallback(â˜ƒ, â˜ƒ);
      GLFW.glfwSetDropCallback(â˜ƒ, â˜ƒ);
   }

   public static void grabOrReleaseMouse(long var0, int var2, double var3, double var5) {
      GLFW.glfwSetCursorPos(â˜ƒ, â˜ƒ, â˜ƒ);
      GLFW.glfwSetInputMode(â˜ƒ, 208897, â˜ƒ);
   }
   ...
   public static void updateRawMouseInput(long var0, boolean var2) {
      if (isRawMouseInputSupported()) {
         GLFW.glfwSetInputMode(â˜ƒ, GLFW_RAW_MOUSE_MOTION, â˜ƒ ? 1 : 0);
      }
   }

   static {
      Lookup â˜ƒ = MethodHandles.lookup();
      MethodType â˜ƒx = MethodType.methodType(Boolean.TYPE);
      MethodHandle â˜ƒxx = null;
      int â˜ƒxxx = 0;

      try {
         â˜ƒxx = â˜ƒ.findStatic(GLFW.class, "glfwRawMouseMotionSupported", â˜ƒx);
         MethodHandle â˜ƒxxxx = â˜ƒ.findStaticGetter(GLFW.class, "GLFW_RAW_MOUSE_MOTION", Integer.TYPE);
         â˜ƒxxx = (int)â˜ƒxxxx.invokeExact();
      } catch (NoSuchFieldException | NoSuchMethodException var5) {
      } catch (Throwable var6) {
         throw new RuntimeException(var6);
      }

      GLFW_RAW_MOUSE_MOTION_SUPPORTED = â˜ƒxx;
      GLFW_RAW_MOUSE_MOTION = â˜ƒxxx;
   ...
   public static final class Key {
      ...
      Key(String var1, InputConstants.Type var2, int var3) {
         ...
         this.type = â˜ƒ;
         this.value = â˜ƒ;
         this.displayName = new LazyLoadedValue<>(() -> (Component)â˜ƒ.displayTextSupplier.apply(â˜ƒ, â˜ƒ));
         NAME_MAP.put(â˜ƒ, this);
      }
      ...
      public boolean equals(Object var1) {
         if (this == â˜ƒ) {
            ...
            InputConstants.Key â˜ƒ = (InputConstants.Key)â˜ƒ;
            return this.value == â˜ƒ.value && this.type == â˜ƒ.type;
         } else {
            return false;
         }
      }
   ...
   public static enum Type {
      KEYSYM("key.keyboard", (var0, var1) -> {
         String â˜ƒ = GLFW.glfwGetKeyName(var0, -1);
         return (Component)(â˜ƒ != null ? new TextComponent(â˜ƒ) : new TranslatableComponent(var1));
      }),
      SCANCODE("scancode", (var0, var1) -> {
         String â˜ƒ = GLFW.glfwGetKeyName(-1, var0);
         return (Component)(â˜ƒ != null ? new TextComponent(â˜ƒ) : new TranslatableComponent(var1));
      }),
      MOUSE("key.mouse", (var0, var1) -> Language.getInstance().has(var1) ? new TranslatableComponent(var1) : new TranslatableComponent("key.mouse", var0 + 1));
      ...
      private static void addKey(InputConstants.Type var0, String var1, int var2) {
         InputConstants.Key â˜ƒ = new InputConstants.Key(â˜ƒ, â˜ƒ, â˜ƒ);
         â˜ƒ.map.put(â˜ƒ, â˜ƒ);
      }

      private Type(String var3, BiFunction<Integer, String, Component> var4) {
         this.defaultPrefix = â˜ƒ;
         this.displayTextSupplier = â˜ƒ;
      }

      public InputConstants.Key getOrCreate(int var1) {
         return this.map.computeIfAbsent(â˜ƒ, var1x -> {
            int â˜ƒ = var1x;
            if (this == MOUSE) {
               â˜ƒ = var1x + 1;
            }

            String â˜ƒ = this.defaultPrefix + "." + â˜ƒ;
            return new InputConstants.Key(â˜ƒ, this, var1x);
         });
      }
