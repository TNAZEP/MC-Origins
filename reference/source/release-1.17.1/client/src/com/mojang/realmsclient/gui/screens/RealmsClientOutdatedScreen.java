...
public class RealmsClientOutdatedScreen extends RealmsScreen {
   ...
   public RealmsClientOutdatedScreen(Screen var1, boolean var2) {
      super(â˜ƒ ? OUTDATED_TITLE : INCOMPATIBLE_TITLE);
      this.lastScreen = â˜ƒ;
      this.outdated = â˜ƒ;
   }
   ...
   public void render(PoseStack var1, int var2, int var3, float var4) {
      ...
      drawCenteredString(â˜ƒ, this.font, this.title, this.width / 2, row(3), 16711680);
      Component[] â˜ƒ = this.outdated ? INCOMPATIBLE_MESSAGES : OUTDATED_MESSAGES;

      for(int â˜ƒx = 0; â˜ƒx < â˜ƒ.length; ++â˜ƒx) {
         drawCenteredString(â˜ƒ, this.font, â˜ƒ[â˜ƒx], this.width / 2, row(5) + â˜ƒx * 12, 16777215);
      }
