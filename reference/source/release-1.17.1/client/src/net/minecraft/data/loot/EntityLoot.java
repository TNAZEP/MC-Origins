...
public class EntityLoot implements Consumer<BiConsumer<ResourceLocation, LootTable.Builder>> {
   ...
   private static LootTable.Builder createSheepTable(ItemLike var0) {
      return LootTable.lootTable()
         .withPool(LootPool.lootPool().setRolls(ConstantValue.exactly(1.0F)).add(LootItem.lootTableItem(â˜ƒ)))
   ...
   public void accept(BiConsumer<ResourceLocation, LootTable.Builder> var1) {
      ...
      for(EntityType<?> â˜ƒx : Registry.ENTITY_TYPE) {
         ResourceLocation â˜ƒxx = â˜ƒx.getDefaultLootTable();
         if (!SPECIAL_LOOT_TABLE_TYPES.contains(â˜ƒx) && â˜ƒx.getCategory() == MobCategory.MISC) {
            if (â˜ƒxx != BuiltInLootTables.EMPTY && this.map.remove(â˜ƒxx) != null) {
               throw new IllegalStateException(
                  String.format("Weird loottable '%s' for '%s', not a LivingEntity so should not have loot", â˜ƒxx, Registry.ENTITY_TYPE.getKey(â˜ƒx))
               );
            }
         } else if (â˜ƒxx != BuiltInLootTables.EMPTY && â˜ƒ.add(â˜ƒxx)) {
            LootTable.Builder â˜ƒxx = (LootTable.Builder)this.map.remove(â˜ƒxx);
            if (â˜ƒxx == null) {
               throw new IllegalStateException(String.format("Missing loottable '%s' for '%s'", â˜ƒxx, Registry.ENTITY_TYPE.getKey(â˜ƒx)));
            }

            â˜ƒ.accept(â˜ƒxx, â˜ƒxx);
         }
      }

      this.map.forEach(â˜ƒ::accept);
   }

   private void add(EntityType<?> var1, LootTable.Builder var2) {
      this.add(â˜ƒ.getDefaultLootTable(), â˜ƒ);
   }
