...
public class ItemParser {
   ...
   public static final DynamicCommandExceptionType ERROR_UNKNOWN_ITEM = new DynamicCommandExceptionType(
      var0 -> new TranslatableComponent("argument.item.id.invalid", var0)
   );
   ...
   public void readItem() throws CommandSyntaxException {
      ...
      ResourceLocation â˜ƒx = ResourceLocation.read(this.reader);
      this.item = (Item)Registry.ITEM.getOptional(â˜ƒx).orElseThrow(() -> {
         this.reader.setCursor(â˜ƒ);
         return ERROR_UNKNOWN_ITEM.createWithContext(this.reader, â˜ƒ.toString());
      });
   }
   ...
   private CompletableFuture<Suggestions> suggestTag(SuggestionsBuilder var1, TagCollection<Item> var2) {
      return SharedSuggestionProvider.suggestResource(â˜ƒ.getAvailableTags(), â˜ƒ.createOffset(this.tagCursor));
   }
   ...
   private CompletableFuture<Suggestions> suggestItemIdOrTag(SuggestionsBuilder var1, TagCollection<Item> var2) {
      ...
      return SharedSuggestionProvider.suggestResource(Registry.ITEM.keySet(), â˜ƒ);
   }
