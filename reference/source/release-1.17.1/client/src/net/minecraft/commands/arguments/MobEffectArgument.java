...
public class MobEffectArgument implements ArgumentType<MobEffect> {
   ...
   public static MobEffect getEffect(CommandContext<CommandSourceStack> var0, String var1) {
      return â˜ƒ.getArgument(â˜ƒ, MobEffect.class);
   }

   public MobEffect parse(StringReader var1) throws CommandSyntaxException {
      ResourceLocation â˜ƒ = ResourceLocation.read(â˜ƒ);
      return (MobEffect)Registry.MOB_EFFECT.getOptional(â˜ƒ).orElseThrow(() -> ERROR_UNKNOWN_EFFECT.create(â˜ƒ));
   }

   @Override
   public <S> CompletableFuture<Suggestions> listSuggestions(CommandContext<S> var1, SuggestionsBuilder var2) {
      return SharedSuggestionProvider.suggestResource(Registry.MOB_EFFECT.keySet(), â˜ƒ);
   }
