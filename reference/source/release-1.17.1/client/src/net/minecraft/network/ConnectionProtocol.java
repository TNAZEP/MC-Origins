...
public enum ConnectionProtocol {
   ...
   private ConnectionProtocol(int var3, ConnectionProtocol.ProtocolBuilder var4) {
      this.id = â˜ƒ;
      this.flows = â˜ƒ.flows;
   }

   @Nullable
   public Integer getPacketId(PacketFlow var1, Packet<?> var2) {
      return ((ConnectionProtocol.PacketSet)this.flows.get(â˜ƒ)).getId(â˜ƒ.getClass());
   }

   @Nullable
   public Packet<?> createPacket(PacketFlow var1, int var2, FriendlyByteBuf var3) {
      return ((ConnectionProtocol.PacketSet)this.flows.get(â˜ƒ)).createPacket(â˜ƒ, â˜ƒ);
   }
   ...
   public static ConnectionProtocol getById(int var0) {
      return â˜ƒ >= -1 && â˜ƒ <= 2 ? LOOKUP[â˜ƒ - -1] : null;
   }

   public static ConnectionProtocol getProtocolForPacket(Packet<?> var0) {
      return (ConnectionProtocol)PROTOCOL_BY_PACKET.get(â˜ƒ.getClass());
   }

   static {
      for(ConnectionProtocol â˜ƒ : values()) {
         int â˜ƒx = â˜ƒ.getId();
         if (â˜ƒx < -1 || â˜ƒx > 2) {
            throw new Error("Invalid protocol ID " + â˜ƒx);
         }
         ...
         â˜ƒ.flows
            .forEach(
               (var1, var2) -> var2.getAllPackets()
                     .forEach(
                        var1x -> {
                           if (PROTOCOL_BY_PACKET.containsKey(var1x) && PROTOCOL_BY_PACKET.get(var1x) != â˜ƒ) {
                              throw new IllegalStateException(
                                 "Packet " + var1x + " is already assigned to protocol " + PROTOCOL_BY_PACKET.get(var1x) + " - can't reassign to " + â˜ƒ
                              );
                           } else {
                              PROTOCOL_BY_PACKET.put(var1x, â˜ƒ);
                           }
                        }
                     )
            );
   ...
   static class PacketSet<T extends PacketListener> {
      ...
      public <P extends Packet<T>> ConnectionProtocol.PacketSet<T> addPacket(Class<P> var1, Function<FriendlyByteBuf, P> var2) {
         int â˜ƒ = this.idToDeserializer.size();
         int â˜ƒx = this.classToId.put(â˜ƒ, â˜ƒ);
         if (â˜ƒx != -1) {
            String â˜ƒxx = "Packet " + â˜ƒ + " is already registered to ID " + â˜ƒx;
            LogManager.getLogger().fatal(â˜ƒxx);
            throw new IllegalArgumentException(â˜ƒxx);
         } else {
            this.idToDeserializer.add(â˜ƒ);
            return this;
         }
      }
      ...
      public Integer getId(Class<?> var1) {
         int â˜ƒ = this.classToId.getInt(â˜ƒ);
         return â˜ƒ == -1 ? null : â˜ƒ;
      }

      @Nullable
      public Packet<?> createPacket(int var1, FriendlyByteBuf var2) {
         Function<FriendlyByteBuf, ? extends Packet<T>> â˜ƒ = (Function)this.idToDeserializer.get(â˜ƒ);
         return â˜ƒ != null ? (Packet)â˜ƒ.apply(â˜ƒ) : null;
      }
