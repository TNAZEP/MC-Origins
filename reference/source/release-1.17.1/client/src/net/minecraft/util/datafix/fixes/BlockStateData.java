...
public class BlockStateData {
   ...
   private static final Object2IntMap<Dynamic<?>> ID_BY_OLD = DataFixUtils.make(new Object2IntOpenHashMap<>(), var0 -> var0.defaultReturnValue(-1));
   private static final Object2IntMap<String> ID_BY_OLD_NAME = DataFixUtils.make(new Object2IntOpenHashMap(), var0 -> var0.defaultReturnValue(-1));
   static final String FILTER_ME = "%%FILTER_ME%%";

   private static void register(int var0, String var1, String... var2) {
      Dynamic<?> â˜ƒ = parse(â˜ƒ);
      MAP[â˜ƒ] = â˜ƒ;
      int â˜ƒx = â˜ƒ >> 4;
      if (BLOCK_DEFAULTS[â˜ƒx] == null) {
         BLOCK_DEFAULTS[â˜ƒx] = â˜ƒ;
      }

      for(String â˜ƒ : â˜ƒ) {
         Dynamic<?> â˜ƒx = parse(â˜ƒ);
         String â˜ƒxx = â˜ƒx.get("Name").asString("");
         ID_BY_OLD_NAME.putIfAbsent(â˜ƒxx, â˜ƒ);
         ID_BY_OLD.put(â˜ƒx, â˜ƒ);
      }
   }

   private static void finalizeMaps() {
      for(int â˜ƒ = 0; â˜ƒ < MAP.length; ++â˜ƒ) {
         if (MAP[â˜ƒ] == null) {
            MAP[â˜ƒ] = BLOCK_DEFAULTS[â˜ƒ >> 4];
         }
      }
   }

   public static Dynamic<?> upgradeBlockStateTag(Dynamic<?> var0) {
      int â˜ƒ = ID_BY_OLD.getInt(â˜ƒ);
      if (â˜ƒ >= 0 && â˜ƒ < MAP.length) {
         Dynamic<?> â˜ƒx = MAP[â˜ƒ];
         return â˜ƒx == null ? â˜ƒ : â˜ƒx;
      } else {
         return â˜ƒ;
      }
   }
   ...
   public static String upgradeBlock(String var0) {
      int â˜ƒ = ID_BY_OLD_NAME.getInt(â˜ƒ);
      if (â˜ƒ >= 0 && â˜ƒ < MAP.length) {
         Dynamic<?> â˜ƒx = MAP[â˜ƒ];
         return â˜ƒx == null ? â˜ƒ : â˜ƒx.get("Name").asString("");
      } else {
         return â˜ƒ;
      }
   }
   ...
   public static String upgradeBlock(int var0) {
      if (â˜ƒ >= 0 && â˜ƒ < MAP.length) {
         Dynamic<?> â˜ƒ = MAP[â˜ƒ];
         return â˜ƒ == null ? "minecraft:air" : â˜ƒ.get("Name").asString("");
      } else {
   ...
   public static Dynamic<?> parse(String var0) {
      try {
         return new Dynamic<>(NbtOps.INSTANCE, TagParser.parseTag(â˜ƒ.replace('\'', '"')));
      } catch (Exception var2) {
         LOGGER.error("Parsing {}", â˜ƒ, var2);
