...
public class ItemStackTheFlatteningFix extends DataFix {
   ...
   private static final Set<String> IDS = (Set<String>)MAP.keySet().stream().map(var0 -> var0.substring(0, var0.indexOf(46))).collect(Collectors.toSet());
   ...
   public TypeRewriteRule makeRule() {
      ...
      return this.fixTypeEverywhereTyped("ItemInstanceTheFlatteningFix", â˜ƒ, var2x -> {
         ...
         if (!â˜ƒ.isPresent()) {
            ...
            int â˜ƒxx = â˜ƒx.get("Damage").asInt(0);
            String â˜ƒxxx = updateItem((String)((Pair)â˜ƒ.get()).getSecond(), â˜ƒxx);
            if (â˜ƒxxx != null) {
               â˜ƒ = var2x.set(â˜ƒ, Pair.of(References.ITEM_NAME.typeName(), â˜ƒxxx));
            }

            if (DAMAGE_IDS.contains(((Pair)â˜ƒ.get()).getSecond())) {
               Typed<?> â˜ƒ = var2x.getOrCreateTyped(â˜ƒ);
               Dynamic<?> â˜ƒx = â˜ƒ.get(DSL.remainderFinder());
               â˜ƒx = â˜ƒx.set("Damage", â˜ƒx.createInt(â˜ƒxx));
               â˜ƒ = â˜ƒ.set(â˜ƒ, â˜ƒ.set(DSL.remainderFinder(), â˜ƒx));
            }

            return â˜ƒ.set(DSL.remainderFinder(), â˜ƒx.remove("Damage"));
         }
      });
   }
   ...
   public static String updateItem(@Nullable String var0, int var1) {
      if (IDS.contains(â˜ƒ)) {
         String â˜ƒ = (String)MAP.get(â˜ƒ + "." + â˜ƒ);
         return â˜ƒ == null ? (String)MAP.get(â˜ƒ + ".0") : â˜ƒ;
      } else {
         return null;
      }
   }
