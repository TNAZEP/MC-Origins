...
public class ChunkStructuresTemplateRenameFix extends DataFix {
   ...
   public TypeRewriteRule makeRule() {
      ...
      OpticFinder<?> â˜ƒx = â˜ƒ.findField("Children");
      return this.fixTypeEverywhereTyped(
         "ChunkStructuresTemplateRenameFix",
         â˜ƒ,
         var2x -> var2x.updateTyped(â˜ƒ, var2xx -> var2xx.update(DSL.remainderFinder(), var2xxx -> this.fixTag(var2x.get(DSL.remainderFinder()), var2xxx)))
      );
   }

   private Dynamic<?> fixTag(Dynamic<?> var1, Dynamic<?> var2) {
      String â˜ƒ = â˜ƒ.get("id").asString("");
      if (RENAMES.containsKey(â˜ƒ)) {
         Pair<String, ImmutableMap<String, String>> â˜ƒx = RENAMES.get(â˜ƒ);
         if (((String)â˜ƒx.getFirst()).equals(â˜ƒ.get("id").asString(""))) {
            String â˜ƒxx = â˜ƒ.get("Template").asString("");
            â˜ƒ = â˜ƒ.set("Template", â˜ƒ.createString((String)â˜ƒx.getSecond().getOrDefault(â˜ƒxx, â˜ƒxx)));
         }
      }
