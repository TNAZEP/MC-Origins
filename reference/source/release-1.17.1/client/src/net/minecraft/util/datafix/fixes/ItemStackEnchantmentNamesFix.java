...
public class ItemStackEnchantmentNamesFix extends DataFix {
   ...
   protected TypeRewriteRule makeRule() {
      ...
      return this.fixTypeEverywhereTyped(
         "ItemStackEnchantmentFix", â˜ƒ, var2x -> var2x.updateTyped(â˜ƒ, var1x -> var1x.update(DSL.remainderFinder(), this::fixTag))
      );
   }

   private Dynamic<?> fixTag(Dynamic<?> var1) {
      Optional<? extends Dynamic<?>> â˜ƒ = â˜ƒ.get("ench")
         .asStreamOpt()
         .map(var0 -> var0.map(var0x -> var0x.set("id", var0x.createString((String)MAP.getOrDefault(var0x.get("id").asInt(0), "null")))))
         .map(â˜ƒ::createList)
         .result();
      if (â˜ƒ.isPresent()) {
         â˜ƒ = â˜ƒ.remove("ench").set("Enchantments", (Dynamic<?>)â˜ƒ.get());
      }
