               ...
               var0x -> (Dynamic)var0x.getMapValues()
                     .map(var1 -> var0x.createMap((Map<? extends Dynamic<?>, ? extends Dynamic<?>>)var1.entrySet().stream().map(var0xx -> {
                           if (((Dynamic)var0xx.getKey()).asString("").startsWith("key_")) {
                              int â˜ƒ = Integer.parseInt(((Dynamic)var0xx.getValue()).asString(""));
                              if (â˜ƒ < 0) {
                                 int â˜ƒxx = â˜ƒ + 100;
                                 String â˜ƒx;
                                 if (â˜ƒxx == 0) {
                                    ...
                                    â˜ƒx = "key.mouse.right";
                                 } else if (â˜ƒxx == 2) {
                                    â˜ƒx = "key.mouse.middle";
                                 } else {
                                    â˜ƒx = "key.mouse." + (â˜ƒxx + 1);
                                 }
         
                                 return Pair.of((Dynamic)var0xx.getKey(), ((Dynamic)var0xx.getValue()).createString(â˜ƒx));
                              } else {
                                 String â˜ƒ = (String)MAP.getOrDefault(â˜ƒ, "key.unknown");
                                 return Pair.of((Dynamic)var0xx.getKey(), ((Dynamic)var0xx.getValue()).createString(â˜ƒ));
                              }
                           } else {
                              return Pair.of((Dynamic)var0xx.getKey(), (Dynamic)var0xx.getValue());
                           }
