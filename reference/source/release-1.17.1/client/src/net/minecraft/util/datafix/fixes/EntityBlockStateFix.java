...
public class EntityBlockStateFix extends DataFix {
   ...
   public static int getBlockId(String var0) {
      Integer â˜ƒ = (Integer)MAP.get(â˜ƒ);
      return â˜ƒ == null ? 0 : â˜ƒ;
   }
   ...
   public TypeRewriteRule makeRule() {
      ...
      Function<Typed<?>, Typed<?>> â˜ƒxx = var1x -> this.updateBlockToBlockState(var1x, "DisplayTile", "DisplayData", "DisplayState");
      Function<Typed<?>, Typed<?>> â˜ƒxxx = var1x -> this.updateBlockToBlockState(var1x, "inTile", "inData", "inBlockState");
      Type<Pair<Either<Pair<String, Either<Integer, String>>, Unit>, Dynamic<?>>> â˜ƒxxxx = DSL.and(
      ...
      Function<Typed<?>, Typed<?>> â˜ƒxxxxx = var1x -> var1x.update(â˜ƒ.finder(), DSL.remainderType(), Pair::getSecond);
      return this.fixTypeEverywhereTyped("EntityBlockStateFix", â˜ƒ.getType(References.ENTITY), â˜ƒx.getType(References.ENTITY), var4x -> {
         ...
         var4x = this.updateEntity(var4x, "minecraft:arrow", â˜ƒ);
         var4x = this.updateEntity(var4x, "minecraft:spectral_arrow", â˜ƒ);
         var4x = this.updateEntity(var4x, "minecraft:egg", â˜ƒ);
         var4x = this.updateEntity(var4x, "minecraft:ender_pearl", â˜ƒ);
         var4x = this.updateEntity(var4x, "minecraft:fireball", â˜ƒ);
         var4x = this.updateEntity(var4x, "minecraft:potion", â˜ƒ);
         var4x = this.updateEntity(var4x, "minecraft:small_fireball", â˜ƒ);
         var4x = this.updateEntity(var4x, "minecraft:snowball", â˜ƒ);
         var4x = this.updateEntity(var4x, "minecraft:wither_skull", â˜ƒ);
         var4x = this.updateEntity(var4x, "minecraft:xp_bottle", â˜ƒ);
         var4x = this.updateEntity(var4x, "minecraft:commandblock_minecart", â˜ƒ);
         var4x = this.updateEntity(var4x, "minecraft:minecart", â˜ƒ);
         var4x = this.updateEntity(var4x, "minecraft:chest_minecart", â˜ƒ);
         var4x = this.updateEntity(var4x, "minecraft:furnace_minecart", â˜ƒ);
         var4x = this.updateEntity(var4x, "minecraft:tnt_minecart", â˜ƒ);
         var4x = this.updateEntity(var4x, "minecraft:hopper_minecart", â˜ƒ);
         return this.updateEntity(var4x, "minecraft:spawner_minecart", â˜ƒ);
      });
   }

   private Typed<?> updateFallingBlock(Typed<?> var1) {
      Type<Either<Pair<String, Either<Integer, String>>, Unit>> â˜ƒ = DSL.optional(
         DSL.field("Block", DSL.named(References.BLOCK_NAME.typeName(), DSL.or(DSL.intType(), NamespacedSchema.namespacedString())))
      );
      Type<Either<Pair<String, Dynamic<?>>, Unit>> â˜ƒx = DSL.optional(
         DSL.field("BlockState", DSL.named(References.BLOCK_STATE.typeName(), DSL.remainderType()))
      );
      Dynamic<?> â˜ƒxx = â˜ƒ.get(DSL.remainderFinder());
      return â˜ƒ.update(â˜ƒ.finder(), â˜ƒx, var1x -> {
         int â˜ƒ = var1x.map(var0x -> ((Either)var0x.getSecond()).map(var0xx -> var0xx, EntityBlockStateFix::getBlockId), var1xx -> {
            Optional<Number> â˜ƒ = â˜ƒ.get("TileID").asNumber().result();
            return (Integer)â˜ƒ.map(Number::intValue).orElseGet(() -> â˜ƒ.get("Tile").asByte((byte)0) & 0xFF);
         });
         int â˜ƒx = â˜ƒ.get("Data").asInt(0) & 15;
         return Either.left(Pair.of(References.BLOCK_STATE.typeName(), BlockStateData.getTag(â˜ƒ << 4 | â˜ƒx)));
      }).set(DSL.remainderFinder(), â˜ƒxx.remove("Data").remove("TileID").remove("Tile"));
   }

   private Typed<?> updateBlockToBlockState(Typed<?> var1, String var2, String var3, String var4) {
      Type<Pair<String, Either<Integer, String>>> â˜ƒ = DSL.field(
         â˜ƒ, DSL.named(References.BLOCK_NAME.typeName(), DSL.or(DSL.intType(), NamespacedSchema.namespacedString()))
      );
      Type<Pair<String, Dynamic<?>>> â˜ƒx = DSL.field(â˜ƒ, DSL.named(References.BLOCK_STATE.typeName(), DSL.remainderType()));
      Dynamic<?> â˜ƒxx = â˜ƒ.getOrCreate(DSL.remainderFinder());
      return â˜ƒ.update(â˜ƒ.finder(), â˜ƒx, var2x -> {
         int â˜ƒ = ((Either)var2x.getSecond()).map(var0x -> var0x, EntityBlockStateFix::getBlockId);
         int â˜ƒx = â˜ƒ.get(â˜ƒ).asInt(0) & 15;
         return Pair.of(References.BLOCK_STATE.typeName(), BlockStateData.getTag(â˜ƒ << 4 | â˜ƒx));
      }).set(DSL.remainderFinder(), â˜ƒxx.remove(â˜ƒ));
   }
   ...
   private Typed<?> updateEntity(Typed<?> var1, String var2, Function<Typed<?>, Typed<?>> var3) {
      ...
      return â˜ƒ.updateTyped(DSL.namedChoice(â˜ƒ, â˜ƒ), â˜ƒx, â˜ƒ);
   }
}
