...
public class ForcePoiRebuild extends DataFix {
   ...
   protected TypeRewriteRule makeRule() {
      ...
      if (!Objects.equals(â˜ƒ, this.getInputSchema().getType(References.POI_CHUNK))) {
         ...
         return this.fixTypeEverywhere("POI rebuild", â˜ƒ, var0 -> var0x -> var0x.mapSecond(ForcePoiRebuild::cap));
      }
   }

   private static <T> Dynamic<T> cap(Dynamic<T> var0) {
      return â˜ƒ.update("Sections", var0x -> var0x.updateMapValues(var0xx -> var0xx.mapSecond(var0xxx -> var0xxx.remove("Valid"))));
   }
}
