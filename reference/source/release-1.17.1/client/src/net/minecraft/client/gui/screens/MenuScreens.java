...
public class MenuScreens {
   ...
   public static <T extends AbstractContainerMenu> void create(@Nullable MenuType<T> var0, Minecraft var1, int var2, Component var3) {
      if (â˜ƒ == null) {
         LOGGER.warn("Trying to open invalid screen with name: {}", â˜ƒ.getString());
      } else {
         MenuScreens.ScreenConstructor<T, ?> â˜ƒ = getConstructor(â˜ƒ);
         if (â˜ƒ == null) {
            LOGGER.warn("Failed to create screen for menu type: {}", Registry.MENU.getKey(â˜ƒ));
         } else {
            â˜ƒ.fromPacket(â˜ƒ, â˜ƒ, â˜ƒ, â˜ƒ);
         }
      }
   }
   ...
   private static <T extends AbstractContainerMenu> MenuScreens.ScreenConstructor<T, ?> getConstructor(MenuType<T> var0) {
      return (MenuScreens.ScreenConstructor<T, ?>)SCREENS.get(â˜ƒ);
   }
   ...
   private static <M extends AbstractContainerMenu, U extends Screen & MenuAccess<M>> void register(
      ...
      MenuScreens.ScreenConstructor<?, ?> â˜ƒ = (MenuScreens.ScreenConstructor)SCREENS.put(â˜ƒ, â˜ƒ);
      if (â˜ƒ != null) {
         throw new IllegalStateException("Duplicate registration for " + Registry.MENU.getKey(â˜ƒ));
      }
   }
   ...
   public static boolean selfTest() {
      ...
      for(MenuType<?> â˜ƒx : Registry.MENU) {
         if (!SCREENS.containsKey(â˜ƒx)) {
            LOGGER.debug("Menu {} has no matching screen", Registry.MENU.getKey(â˜ƒx));
            â˜ƒ = true;
         }
      }
   ...
   interface ScreenConstructor<T extends AbstractContainerMenu, U extends Screen & MenuAccess<T>> {
      default void fromPacket(Component var1, MenuType<T> var2, Minecraft var3, int var4) {
         U â˜ƒ = this.create(â˜ƒ.create(â˜ƒ, â˜ƒ.player.getInventory()), â˜ƒ.player.getInventory(), â˜ƒ);
         â˜ƒ.player.containerMenu = â˜ƒ.getMenu();
         â˜ƒ.setScreen(â˜ƒ);
      }
