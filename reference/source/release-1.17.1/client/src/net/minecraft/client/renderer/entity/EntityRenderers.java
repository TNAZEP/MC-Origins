...
public class EntityRenderers {
   ...
   private static final Map<String, EntityRendererProvider<AbstractClientPlayer>> PLAYER_PROVIDERS = ImmutableMap.of(
      "default", var0 -> new PlayerRenderer(var0, false), "slim", var0 -> new PlayerRenderer(var0, true)
   );
   ...
   public static Map<EntityType<?>, EntityRenderer<?>> createEntityRenderers(EntityRendererProvider.Context var0) {
      Builder<EntityType<?>, EntityRenderer<?>> â˜ƒ = ImmutableMap.builder();
   ...
   public static Map<String, EntityRenderer<? extends Player>> createPlayerRenderers(EntityRendererProvider.Context var0) {
      Builder<String, EntityRenderer<? extends Player>> â˜ƒ = ImmutableMap.builder();
   ...
   public static boolean validateRegistrations() {
      ...
      for(EntityType<?> â˜ƒx : Registry.ENTITY_TYPE) {
         if (â˜ƒx != EntityType.PLAYER && !PROVIDERS.containsKey(â˜ƒx)) {
            LOGGER.warn("No renderer registered for {}", Registry.ENTITY_TYPE.getKey(â˜ƒx));
            â˜ƒ = false;
         }
      }
