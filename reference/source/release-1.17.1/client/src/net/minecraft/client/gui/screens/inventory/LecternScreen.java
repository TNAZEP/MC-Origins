...
public class LecternScreen extends BookViewScreen implements MenuAccess<LecternMenu> {
   ...
   protected void createMenuControls() {
      if (this.minecraft.player.mayBuild()) {
         ...
         this.addRenderableWidget(new Button(this.width / 2 + 2, 196, 98, 20, new TranslatableComponent("lectern.take_book"), var1 -> this.sendButtonClick(3)));
      } else {
   ...
   protected boolean forcePage(int var1) {
      if (â˜ƒ != this.menu.getPage()) {
         this.sendButtonClick(100 + â˜ƒ);
         return true;
      } else {
         return false;
      }
   ...
   private void sendButtonClick(int var1) {
      this.minecraft.gameMode.handleInventoryButtonClick(this.menu.containerId, â˜ƒ);
   }
   ...
   void bookChanged() {
      ItemStack â˜ƒ = this.menu.getBook();
      this.setBookAccess(BookViewScreen.BookAccess.fromItem(â˜ƒ));
   }
