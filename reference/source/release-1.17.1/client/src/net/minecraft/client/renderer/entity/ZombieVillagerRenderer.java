...
public class ZombieVillagerRenderer extends HumanoidMobRenderer<ZombieVillager, ZombieVillagerModel<ZombieVillager>> {
   ...
   public ZombieVillagerRenderer(EntityRendererProvider.Context var1) {
      super(â˜ƒ, new ZombieVillagerModel<>(â˜ƒ.bakeLayer(ModelLayers.ZOMBIE_VILLAGER)), 0.5F);
      this.addLayer(
         new HumanoidArmorLayer<>(
            this,
            new ZombieVillagerModel(â˜ƒ.bakeLayer(ModelLayers.ZOMBIE_VILLAGER_INNER_ARMOR)),
            new ZombieVillagerModel(â˜ƒ.bakeLayer(ModelLayers.ZOMBIE_VILLAGER_OUTER_ARMOR))
         )
      );
      this.addLayer(new VillagerProfessionLayer<>(this, â˜ƒ.getResourceManager(), "zombie_villager"));
   }
