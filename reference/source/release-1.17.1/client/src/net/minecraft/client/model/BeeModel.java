...
public class BeeModel<T extends Bee> extends AgeableListModel<T> {
   ...
   public BeeModel(ModelPart var1) {
      ...
      this.bone = â˜ƒ.getChild("bone");
      ModelPart â˜ƒ = this.bone.getChild("body");
      this.stinger = â˜ƒ.getChild("stinger");
      this.leftAntenna = â˜ƒ.getChild("left_antenna");
      this.rightAntenna = â˜ƒ.getChild("right_antenna");
   ...
   public static LayerDefinition createBodyLayer() {
      float â˜ƒ = 19.0F;
      MeshDefinition â˜ƒx = new MeshDefinition();
      PartDefinition â˜ƒxx = â˜ƒx.getRoot();
      PartDefinition â˜ƒxxx = â˜ƒxx.addOrReplaceChild("bone", CubeListBuilder.create(), PartPose.offset(0.0F, 19.0F, 0.0F));
      ...
      â˜ƒxxxx.addOrReplaceChild("stinger", CubeListBuilder.create().texOffs(26, 7).addBox(0.0F, -1.0F, 5.0F, 0.0F, 1.0F, 2.0F), PartPose.ZERO);
      ...
      CubeDeformation â˜ƒxxxxx = new CubeDeformation(0.001F);
      â˜ƒxxx.addOrReplaceChild(
         "right_wing",
         CubeListBuilder.create().texOffs(0, 18).addBox(-9.0F, 0.0F, 0.0F, 9.0F, 0.0F, 6.0F, â˜ƒxxxxx),
         PartPose.offsetAndRotation(-1.5F, -4.0F, -3.0F, 0.0F, -0.2618F, 0.0F)
      );
      â˜ƒxxx.addOrReplaceChild(
         "left_wing",
         CubeListBuilder.create().texOffs(0, 18).mirror().addBox(0.0F, 0.0F, 0.0F, 9.0F, 0.0F, 6.0F, â˜ƒxxxxx),
         PartPose.offsetAndRotation(1.5F, -4.0F, -3.0F, 0.0F, 0.2618F, 0.0F)
      );
      â˜ƒxxx.addOrReplaceChild(
      ...
      â˜ƒxxx.addOrReplaceChild("back_legs", CubeListBuilder.create().addBox("back_legs", -5.0F, 0.0F, 0.0F, 7, 2, 0, 26, 5), PartPose.offset(1.5F, 3.0F, 2.0F));
      return LayerDefinition.create(â˜ƒx, 64, 64);
   }

   public void prepareMobModel(T var1, float var2, float var3, float var4) {
      super.prepareMobModel(â˜ƒ, â˜ƒ, â˜ƒ, â˜ƒ);
      this.rollAmount = â˜ƒ.getRollAmount(â˜ƒ);
      this.stinger.visible = !â˜ƒ.hasStung();
   }
   ...
   public void setupAnim(T var1, float var2, float var3, float var4, float var5, float var6) {
      ...
      this.bone.xRot = 0.0F;
      boolean â˜ƒ = â˜ƒ.isOnGround() && â˜ƒ.getDeltaMovement().lengthSqr() < 1.0E-7;
      if (â˜ƒ) {
         ...
         float â˜ƒ = â˜ƒ * 120.32113F * (float) (Math.PI / 180.0);
         this.rightWing.yRot = 0.0F;
         this.rightWing.zRot = Mth.cos(â˜ƒ) * (float) Math.PI * 0.15F;
      ...
      if (!â˜ƒ.isAngry()) {
         ...
         this.bone.zRot = 0.0F;
         if (!â˜ƒ) {
            float â˜ƒ = Mth.cos(â˜ƒ * 0.18F);
            this.bone.xRot = 0.1F + â˜ƒ * (float) Math.PI * 0.025F;
            this.leftAntenna.xRot = â˜ƒ * (float) Math.PI * 0.03F;
            this.rightAntenna.xRot = â˜ƒ * (float) Math.PI * 0.03F;
            this.frontLeg.xRot = -â˜ƒ * (float) Math.PI * 0.1F + (float) (Math.PI / 8);
            this.backLeg.xRot = -â˜ƒ * (float) Math.PI * 0.05F + (float) (Math.PI / 4);
            this.bone.y = 19.0F - Mth.cos(â˜ƒ * 0.18F) * 0.9F;
         }
      }
