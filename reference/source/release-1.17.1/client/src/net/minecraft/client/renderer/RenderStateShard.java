...
public abstract class RenderStateShard {
   ...
   protected static final RenderStateShard.LayeringStateShard VIEW_OFFSET_Z_LAYERING = new RenderStateShard.LayeringStateShard(
      "view_offset_z_layering", () -> {
         ...
         â˜ƒ.pushPose();
         â˜ƒ.scale(0.99975586F, 0.99975586F, 0.99975586F);
   ...
   private static void setupGlintTexturing(float var0) {
      long â˜ƒ = Util.getMillis() * 8L;
      float â˜ƒx = (float)(â˜ƒ % 110000L) / 110000.0F;
      float â˜ƒxx = (float)(â˜ƒ % 30000L) / 30000.0F;
      Matrix4f â˜ƒxxx = Matrix4f.createTranslateMatrix(-â˜ƒx, â˜ƒxx, 0.0F);
      â˜ƒxxx.multiply(Vector3f.ZP.rotationDegrees(10.0F));
      â˜ƒxxx.multiply(Matrix4f.createScaleMatrix(â˜ƒ, â˜ƒ, â˜ƒ));
      RenderSystem.setTextureMatrix(â˜ƒxxx);
   }
   ...
   protected static class LineStateShard extends RenderStateShard {
      ...
      public LineStateShard(OptionalDouble var1) {
         super("line_width", () -> {
            if (!Objects.equals(â˜ƒ, OptionalDouble.of(1.0))) {
               if (â˜ƒ.isPresent()) {
                  RenderSystem.lineWidth((float)â˜ƒ.getAsDouble());
               } else {
   ...
   protected static class MultiTextureStateShard extends RenderStateShard.EmptyTextureStateShard {
      ...
      MultiTextureStateShard(ImmutableList<Triple<ResourceLocation, Boolean, Boolean>> var1) {
         super(() -> {
            ...
            for(Triple<ResourceLocation, Boolean, Boolean> â˜ƒx : â˜ƒ) {
               TextureManager â˜ƒxx = Minecraft.getInstance().getTextureManager();
               â˜ƒxx.getTexture(â˜ƒx.getLeft()).setFilter(â˜ƒx.getMiddle(), â˜ƒx.getRight());
               RenderSystem.setShaderTexture(â˜ƒ++, â˜ƒx.getLeft());
            }
         }, () -> {
         });
         this.cutoutTexture = â˜ƒ.stream().findFirst().map(Triple::getLeft);
      }
      ...
      public static final class Builder {
         ...
         public RenderStateShard.MultiTextureStateShard.Builder add(ResourceLocation var1, boolean var2, boolean var3) {
            this.builder.add(Triple.of(â˜ƒ, â˜ƒ, â˜ƒ));
            return this;
         }
   ...
   protected static final class OffsetTexturingStateShard extends RenderStateShard.TexturingStateShard {
      public OffsetTexturingStateShard(float var1, float var2) {
         super("offset_texturing", () -> RenderSystem.setTextureMatrix(Matrix4f.createTranslateMatrix(â˜ƒ, â˜ƒ, 0.0F)), () -> RenderSystem.resetTextureMatrix());
      }
   }
   ...
   protected static class ShaderStateShard extends RenderStateShard {
      ...
      public ShaderStateShard(Supplier<ShaderInstance> var1) {
         super("shader", () -> RenderSystem.setShader(â˜ƒ), () -> {
         });
         this.shader = Optional.of(â˜ƒ);
      }
   ...
   protected static class TextureStateShard extends RenderStateShard.EmptyTextureStateShard {
      ...
      public TextureStateShard(ResourceLocation var1, boolean var2, boolean var3) {
         super(() -> {
            ...
            â˜ƒ.getTexture(â˜ƒ).setFilter(â˜ƒ, â˜ƒ);
            RenderSystem.setShaderTexture(0, â˜ƒ);
         }, () -> {
         });
         this.texture = Optional.of(â˜ƒ);
         this.blur = â˜ƒ;
         this.mipmap = â˜ƒ;
      }
   ...
   protected static class WriteMaskStateShard extends RenderStateShard {
      ...
      public WriteMaskStateShard(boolean var1, boolean var2) {
         super("write_mask_state", () -> {
            if (!â˜ƒ) {
               RenderSystem.depthMask(â˜ƒ);
            }

            if (!â˜ƒ) {
               RenderSystem.colorMask(â˜ƒ, â˜ƒ, â˜ƒ, â˜ƒ);
            }
         }, () -> {
            if (!â˜ƒ) {
