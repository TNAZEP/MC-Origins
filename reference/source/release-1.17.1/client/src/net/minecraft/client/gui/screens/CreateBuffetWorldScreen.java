...
public class CreateBuffetWorldScreen extends Screen {
   ...
   public CreateBuffetWorldScreen(Screen var1, RegistryAccess var2, Consumer<Biome> var3, Biome var4) {
      ...
      this.biome = â˜ƒ;
      this.biomes = â˜ƒ.registryOrThrow(Registry.BIOME_REGISTRY);
   }
      ...
      this.list
         .setSelected(
            (CreateBuffetWorldScreen.BiomeList.Entry)this.list
               .children()
               .stream()
               .filter(var1 -> Objects.equals(var1.biome, this.biome))
               .findFirst()
               .orElse(null)
         );
   }
   ...
   public void render(PoseStack var1, int var2, int var3, float var4) {
      ...
      drawCenteredString(â˜ƒ, this.font, BIOME_SELECT_INFO, this.width / 2, 28, 10526880);
   ...
   class BiomeList extends ObjectSelectionList<CreateBuffetWorldScreen.BiomeList.Entry> {
      BiomeList() {
         ...
         CreateBuffetWorldScreen.this.biomes
            .entrySet()
            .stream()
            .sorted(Comparator.comparing(var0 -> ((ResourceKey)var0.getKey()).location().toString()))
            .forEach(var1x -> this.addEntry(new CreateBuffetWorldScreen.BiomeList.Entry((Biome)var1x.getValue())));
      }
      ...
      public void setSelected(@Nullable CreateBuffetWorldScreen.BiomeList.Entry var1) {
         ...
         if (â˜ƒ != null) {
            CreateBuffetWorldScreen.this.biome = â˜ƒ.biome;
         }
      ...
      class Entry extends ObjectSelectionList.Entry<CreateBuffetWorldScreen.BiomeList.Entry> {
         ...
         public Entry(Biome var2) {
            this.biome = â˜ƒ;
            ResourceLocation â˜ƒ = CreateBuffetWorldScreen.this.biomes.getKey(â˜ƒ);
            String â˜ƒx = "biome." + â˜ƒ.getNamespace() + "." + â˜ƒ.getPath();
            if (Language.getInstance().has(â˜ƒx)) {
               this.name = new TranslatableComponent(â˜ƒx);
            } else {
               this.name = new TextComponent(â˜ƒ.toString());
            }
         }
         ...
         public void render(PoseStack var1, int var2, int var3, int var4, int var5, int var6, int var7, int var8, boolean var9, float var10) {
            GuiComponent.drawString(â˜ƒ, CreateBuffetWorldScreen.this.font, this.name, â˜ƒ + 5, â˜ƒ + 2, 16777215);
         }
