...
public class GiantMobRenderer extends MobRenderer<Giant, HumanoidModel<Giant>> {
   ...
   public GiantMobRenderer(EntityRendererProvider.Context var1, float var2) {
      super(â˜ƒ, new GiantZombieModel(â˜ƒ.bakeLayer(ModelLayers.GIANT)), 0.5F * â˜ƒ);
      this.scale = â˜ƒ;
      this.addLayer(new ItemInHandLayer<>(this));
      this.addLayer(
         new HumanoidArmorLayer<>(
            this, new GiantZombieModel(â˜ƒ.bakeLayer(ModelLayers.GIANT_INNER_ARMOR)), new GiantZombieModel(â˜ƒ.bakeLayer(ModelLayers.GIANT_OUTER_ARMOR))
         )
      );
   }
