...
public class PackSelectionModel {
   ...
   public PackSelectionModel(Runnable var1, Function<Pack, ResourceLocation> var2, PackRepository var3, Consumer<PackRepository> var4) {
      this.onListChanged = â˜ƒ;
      this.iconGetter = â˜ƒ;
      this.repository = â˜ƒ;
      this.selected = Lists.<Pack>newArrayList(â˜ƒ.getSelectedPacks());
      Collections.reverse(this.selected);
      this.unselected = Lists.<Pack>newArrayList(â˜ƒ.getAvailablePacks());
   ...
   abstract class EntryBase implements PackSelectionModel.Entry {
      ...
      protected void move(int var1) {
         List<Pack> â˜ƒ = this.getSelfList();
         int â˜ƒx = â˜ƒ.indexOf(this.pack);
         â˜ƒ.remove(â˜ƒx);
         â˜ƒ.add(â˜ƒx + â˜ƒ, this.pack);
         PackSelectionModel.this.onListChanged.run();
      }
      ...
      public boolean canMoveUp() {
         List<Pack> â˜ƒ = this.getSelfList();
         int â˜ƒx = â˜ƒ.indexOf(this.pack);
         return â˜ƒx > 0 && !((Pack)â˜ƒ.get(â˜ƒx - 1)).isFixedPosition();
      }
      ...
      public boolean canMoveDown() {
         List<Pack> â˜ƒ = this.getSelfList();
         int â˜ƒx = â˜ƒ.indexOf(this.pack);
         return â˜ƒx >= 0 && â˜ƒx < â˜ƒ.size() - 1 && !((Pack)â˜ƒ.get(â˜ƒx + 1)).isFixedPosition();
      }
