...
public enum NoteBlockInstrument implements StringRepresentable {
   ...
   public static NoteBlockInstrument byState(BlockState var0) {
      if (â˜ƒ.is(Blocks.CLAY)) {
         return FLUTE;
      } else if (â˜ƒ.is(Blocks.GOLD_BLOCK)) {
         return BELL;
      } else if (â˜ƒ.is(BlockTags.WOOL)) {
         return GUITAR;
      } else if (â˜ƒ.is(Blocks.PACKED_ICE)) {
         return CHIME;
      } else if (â˜ƒ.is(Blocks.BONE_BLOCK)) {
         return XYLOPHONE;
      } else if (â˜ƒ.is(Blocks.IRON_BLOCK)) {
         return IRON_XYLOPHONE;
      } else if (â˜ƒ.is(Blocks.SOUL_SAND)) {
         return COW_BELL;
      } else if (â˜ƒ.is(Blocks.PUMPKIN)) {
         return DIDGERIDOO;
      } else if (â˜ƒ.is(Blocks.EMERALD_BLOCK)) {
         return BIT;
      } else if (â˜ƒ.is(Blocks.HAY_BLOCK)) {
         return BANJO;
      } else if (â˜ƒ.is(Blocks.GLOWSTONE)) {
         return PLING;
      } else {
         Material â˜ƒ = â˜ƒ.getMaterial();
         if (â˜ƒ == Material.STONE) {
            return BASEDRUM;
         } else if (â˜ƒ == Material.SAND) {
            return SNARE;
         } else if (â˜ƒ == Material.GLASS) {
            return HAT;
         } else {
            return â˜ƒ != Material.WOOD && â˜ƒ != Material.NETHER_WOOD ? HARP : BASS;
         }
      }
   }
}
