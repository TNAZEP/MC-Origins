...
public class VillagerTrades {
   ...
   private static Int2ObjectMap<VillagerTrades.ItemListing[]> toIntMap(ImmutableMap<Integer, VillagerTrades.ItemListing[]> var0) {
      return new Int2ObjectOpenHashMap<>(â˜ƒ);
   }
   ...
   static class DyedArmorForEmeralds implements VillagerTrades.ItemListing {
      ...
      public MerchantOffer getOffer(Entity var1, Random var2) {
         ItemStack â˜ƒ = new ItemStack(Items.EMERALD, this.value);
         ItemStack â˜ƒx = new ItemStack(this.item);
         if (this.item instanceof DyeableArmorItem) {
            List<DyeItem> â˜ƒxx = Lists.<DyeItem>newArrayList();
            â˜ƒxx.add(getRandomDye(â˜ƒ));
            if (â˜ƒ.nextFloat() > 0.7F) {
               â˜ƒxx.add(getRandomDye(â˜ƒ));
            }

            if (â˜ƒ.nextFloat() > 0.8F) {
               â˜ƒxx.add(getRandomDye(â˜ƒ));
            }

            â˜ƒx = DyeableLeatherItem.dyeArmor(â˜ƒx, â˜ƒxx);
         }

         return new MerchantOffer(â˜ƒ, â˜ƒx, this.maxUses, this.villagerXp, 0.2F);
      }

      private static DyeItem getRandomDye(Random var0) {
         return DyeItem.byColor(DyeColor.byId(â˜ƒ.nextInt(16)));
      }
   }
   ...
   static class EmeraldForItems implements VillagerTrades.ItemListing {
      ...
      @Override
      public MerchantOffer getOffer(Entity var1, Random var2) {
         ItemStack â˜ƒ = new ItemStack(this.item, this.cost);
         return new MerchantOffer(â˜ƒ, new ItemStack(Items.EMERALD), this.maxUses, this.villagerXp, this.priceMultiplier);
      }
   }
   ...
   static class EmeraldsForVillagerTypeItem implements VillagerTrades.ItemListing {
      ...
      public EmeraldsForVillagerTypeItem(int var1, int var2, int var3, Map<VillagerType, Item> var4) {
         Registry.VILLAGER_TYPE.stream().filter(var1x -> !â˜ƒ.containsKey(var1x)).findAny().ifPresent(var0 -> {
      ...
      @Override
      public MerchantOffer getOffer(Entity var1, Random var2) {
         if (â˜ƒ instanceof VillagerDataHolder) {
            ItemStack â˜ƒ = new ItemStack((ItemLike)this.trades.get(((VillagerDataHolder)â˜ƒ).getVillagerData().getType()), this.cost);
            return new MerchantOffer(â˜ƒ, new ItemStack(Items.EMERALD), this.maxUses, this.villagerXp, 0.05F);
         } else {
            return null;
         }
      }
   ...
   static class EnchantBookForEmeralds implements VillagerTrades.ItemListing {
      ...
      @Override
      public MerchantOffer getOffer(Entity var1, Random var2) {
         List<Enchantment> â˜ƒ = (List)Registry.ENCHANTMENT.stream().filter(Enchantment::isTradeable).collect(Collectors.toList());
         Enchantment â˜ƒx = (Enchantment)â˜ƒ.get(â˜ƒ.nextInt(â˜ƒ.size()));
         int â˜ƒxx = Mth.nextInt(â˜ƒ, â˜ƒx.getMinLevel(), â˜ƒx.getMaxLevel());
         ItemStack â˜ƒxxx = EnchantedBookItem.createForEnchantment(new EnchantmentInstance(â˜ƒx, â˜ƒxx));
         int â˜ƒxxxx = 2 + â˜ƒ.nextInt(5 + â˜ƒxx * 10) + 3 * â˜ƒxx;
         if (â˜ƒx.isTreasureOnly()) {
            â˜ƒxxxx *= 2;
         }
         ...
         return new MerchantOffer(new ItemStack(Items.EMERALD, â˜ƒxxxx), new ItemStack(Items.BOOK), â˜ƒxxx, 12, this.villagerXp, 0.2F);
      }
   }
   ...
   static class EnchantedItemForEmeralds implements VillagerTrades.ItemListing {
      ...
      public EnchantedItemForEmeralds(Item var1, int var2, int var3, int var4) {
         this(â˜ƒ, â˜ƒ, â˜ƒ, â˜ƒ, 0.05F);
      }

      public EnchantedItemForEmeralds(Item var1, int var2, int var3, int var4, float var5) {
         this.itemStack = new ItemStack(â˜ƒ);
         this.baseEmeraldCost = â˜ƒ;
         this.maxUses = â˜ƒ;
         this.villagerXp = â˜ƒ;
      ...
      @Override
      public MerchantOffer getOffer(Entity var1, Random var2) {
         int â˜ƒ = 5 + â˜ƒ.nextInt(15);
         ItemStack â˜ƒx = EnchantmentHelper.enchantItem(â˜ƒ, new ItemStack(this.itemStack.getItem()), â˜ƒ, false);
         int â˜ƒxx = Math.min(this.baseEmeraldCost + â˜ƒ, 64);
         ItemStack â˜ƒxxx = new ItemStack(Items.EMERALD, â˜ƒxx);
         return new MerchantOffer(â˜ƒxxx, â˜ƒx, this.maxUses, this.villagerXp, this.priceMultiplier);
      }
   }
   ...
   static class ItemsAndEmeraldsToItems implements VillagerTrades.ItemListing {
      ...
      public ItemsAndEmeraldsToItems(ItemLike var1, int var2, Item var3, int var4, int var5, int var6) {
         this(â˜ƒ, â˜ƒ, 1, â˜ƒ, â˜ƒ, â˜ƒ, â˜ƒ);
      }

      public ItemsAndEmeraldsToItems(ItemLike var1, int var2, int var3, Item var4, int var5, int var6, int var7) {
         this.fromItem = new ItemStack(â˜ƒ);
         this.fromCount = â˜ƒ;
         this.emeraldCost = â˜ƒ;
         this.toItem = new ItemStack(â˜ƒ);
         this.toCount = â˜ƒ;
         this.maxUses = â˜ƒ;
         this.villagerXp = â˜ƒ;
         this.priceMultiplier = 0.05F;
   ...
   static class ItemsForEmeralds implements VillagerTrades.ItemListing {
      ...
      public ItemsForEmeralds(Block var1, int var2, int var3, int var4, int var5) {
         this(new ItemStack(â˜ƒ), â˜ƒ, â˜ƒ, â˜ƒ, â˜ƒ);
      }

      public ItemsForEmeralds(Item var1, int var2, int var3, int var4) {
         this(new ItemStack(â˜ƒ), â˜ƒ, â˜ƒ, 12, â˜ƒ);
      }
   ...
   static class SuspisciousStewForEmerald implements VillagerTrades.ItemListing {
      ...
      @Override
      public MerchantOffer getOffer(Entity var1, Random var2) {
         ItemStack â˜ƒ = new ItemStack(Items.SUSPICIOUS_STEW, 1);
         SuspiciousStewItem.saveMobEffect(â˜ƒ, this.effect, this.duration);
         return new MerchantOffer(new ItemStack(Items.EMERALD, 1), â˜ƒ, 12, this.xp, this.priceMultiplier);
      }
   }
   ...
   static class TippedArrowForItemsAndEmeralds implements VillagerTrades.ItemListing {
      ...
      @Override
      public MerchantOffer getOffer(Entity var1, Random var2) {
         ItemStack â˜ƒ = new ItemStack(Items.EMERALD, this.emeraldCost);
         List<Potion> â˜ƒx = (List)Registry.POTION
            .stream()
            .filter(var0 -> !var0.getEffects().isEmpty() && PotionBrewing.isBrewablePotion(var0))
            .collect(Collectors.toList());
         Potion â˜ƒxx = (Potion)â˜ƒx.get(â˜ƒ.nextInt(â˜ƒx.size()));
         ItemStack â˜ƒxxx = PotionUtils.setPotion(new ItemStack(this.toItem.getItem(), this.toCount), â˜ƒxx);
         return new MerchantOffer(â˜ƒ, new ItemStack(this.fromItem, this.fromCount), â˜ƒxxx, this.maxUses, this.villagerXp, this.priceMultiplier);
      }
   }
   ...
   static class TreasureMapForEmeralds implements VillagerTrades.ItemListing {
      ...
      @Override
      public MerchantOffer getOffer(Entity var1, Random var2) {
         if (!(â˜ƒ.level instanceof ServerLevel)) {
            return null;
         } else {
            ServerLevel â˜ƒ = (ServerLevel)â˜ƒ.level;
            BlockPos â˜ƒx = â˜ƒ.findNearestMapFeature(this.destination, â˜ƒ.blockPosition(), 100, true);
            if (â˜ƒx != null) {
               ItemStack â˜ƒxx = MapItem.create(â˜ƒ, â˜ƒx.getX(), â˜ƒx.getZ(), (byte)2, true, true);
               MapItem.renderBiomePreviewMap(â˜ƒ, â˜ƒxx);
               MapItemSavedData.addTargetDecoration(â˜ƒxx, â˜ƒx, "+", this.destinationType);
               â˜ƒxx.setHoverName(new TranslatableComponent("filled_map." + this.destination.getFeatureName().toLowerCase(Locale.ROOT)));
               return new MerchantOffer(
                  new ItemStack(Items.EMERALD, this.emeraldCost), new ItemStack(Items.COMPASS), â˜ƒxx, this.maxUses, this.villagerXp, 0.2F
               );
            } else {
               return null;
            }
