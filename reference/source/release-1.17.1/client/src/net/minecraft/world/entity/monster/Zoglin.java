...
public class Zoglin extends Monster implements Enemy, HoglinBase {
   ...
   protected Brain<?> makeBrain(Dynamic<?> var1) {
      Brain<Zoglin> â˜ƒ = this.brainProvider().makeBrain(â˜ƒ);
   ...
   private boolean isTargetable(LivingEntity var1) {
      ...
      return â˜ƒ != EntityType.ZOGLIN && â˜ƒ != EntityType.CREEPER && Sensor.isEntityAttackable(this, â˜ƒ);
   }
   ...
   public boolean hurt(DamageSource var1, float var2) {
      ...
      if (this.level.isClientSide) {
         return false;
      } else if (â˜ƒ && â˜ƒ.getEntity() instanceof LivingEntity â˜ƒ) {
         if (this.canAttack(â˜ƒ) && !BehaviorUtils.isOtherTargetMuchFurtherAwayThanCurrentAttackTarget(this, â˜ƒ, 4.0)) {
            this.setAttackTarget(â˜ƒ);
         }
   ...
   private void setAttackTarget(LivingEntity var1) {
      ...
      this.brain.setMemoryWithExpiry(MemoryModuleType.ATTACK_TARGET, â˜ƒ, 200L);
   }
   ...
   protected void updateActivity() {
      Activity â˜ƒ = (Activity)this.brain.getActiveNonCoreActivity().orElse(null);
      this.brain.setActiveActivityToFirstValid(ImmutableList.of(Activity.FIGHT, Activity.IDLE));
      Activity â˜ƒx = (Activity)this.brain.getActiveNonCoreActivity().orElse(null);
      if (â˜ƒx == Activity.FIGHT && â˜ƒ != Activity.FIGHT) {
         this.playAngrySound();
      }
