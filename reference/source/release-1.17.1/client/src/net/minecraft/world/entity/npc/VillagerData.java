...
public class VillagerData {
   ...
   public static final Codec<VillagerData> CODEC = RecordCodecBuilder.create(
      var0 -> var0.group(
               Registry.VILLAGER_TYPE.fieldOf("type").orElseGet(() -> VillagerType.PLAINS).forGetter(var0x -> var0x.type),
               Registry.VILLAGER_PROFESSION.fieldOf("profession").orElseGet(() -> VillagerProfession.NONE).forGetter(var0x -> var0x.profession),
   ...
   public VillagerData setType(VillagerType var1) {
      return new VillagerData(â˜ƒ, this.profession, this.level);
   }

   public VillagerData setProfession(VillagerProfession var1) {
      return new VillagerData(this.type, â˜ƒ, this.level);
   }

   public VillagerData setLevel(int var1) {
      return new VillagerData(this.type, this.profession, â˜ƒ);
   }

   public static int getMinXpPerLevel(int var0) {
      return canLevelUp(â˜ƒ) ? NEXT_LEVEL_XP_THRESHOLDS[â˜ƒ - 1] : 0;
   }

   public static int getMaxXpPerLevel(int var0) {
      return canLevelUp(â˜ƒ) ? NEXT_LEVEL_XP_THRESHOLDS[â˜ƒ] : 0;
   }
