...
public abstract class AbstractSchoolingFish extends AbstractFish {
   ...
   public void tick() {
      ...
      if (this.hasFollowers() && this.level.random.nextInt(200) == 1) {
         List<? extends AbstractFish> â˜ƒ = this.level.getEntitiesOfClass(this.getClass(), this.getBoundingBox().inflate(8.0, 8.0, 8.0));
         if (â˜ƒ.size() <= 1) {
            this.schoolSize = 1;
         }
      }
   ...
   public void addFollowers(Stream<? extends AbstractSchoolingFish> var1) {
      â˜ƒ.limit((long)(this.getMaxSchoolSize() - this.schoolSize)).filter(var1x -> var1x != this).forEach(var1x -> var1x.startFollowing(this));
   }
   ...
   public SpawnGroupData finalizeSpawn(
      ...
      super.finalizeSpawn(â˜ƒ, â˜ƒ, â˜ƒ, â˜ƒ, â˜ƒ);
      if (â˜ƒ == null) {
         â˜ƒ = new AbstractSchoolingFish.SchoolSpawnGroupData(this);
      } else {
         this.startFollowing(((AbstractSchoolingFish.SchoolSpawnGroupData)â˜ƒ).leader);
      }
