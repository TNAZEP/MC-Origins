...
public class ForestFlowerProvider extends BlockStateProvider {
   ...
   public BlockState getState(Random var1, BlockPos var2) {
      double â˜ƒ = Mth.clamp((1.0 + Biome.BIOME_INFO_NOISE.getValue((double)â˜ƒ.getX() / 48.0, (double)â˜ƒ.getZ() / 48.0, false)) / 2.0, 0.0, 0.9999);
      return FLOWERS[(int)(â˜ƒ * (double)FLOWERS.length)];
   }
}
