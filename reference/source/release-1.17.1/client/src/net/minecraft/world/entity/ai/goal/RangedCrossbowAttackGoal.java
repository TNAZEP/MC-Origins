...
public class RangedCrossbowAttackGoal<T extends Monster & RangedAttackMob & CrossbowAttackMob> extends Goal {
   ...
   public void tick() {
      ...
      if (â˜ƒ != null) {
         boolean â˜ƒx = this.mob.getSensing().hasLineOfSight(â˜ƒ);
         boolean â˜ƒxx = this.seeTime > 0;
         if (â˜ƒx != â˜ƒxx) {
            this.seeTime = 0;
         }
         ...
         double â˜ƒx = this.mob.distanceToSqr(â˜ƒ);
         boolean â˜ƒxx = (â˜ƒx > (double)this.attackRadiusSqr || this.seeTime < 5) && this.attackDelay == 0;
         if (â˜ƒxx) {
            --this.updatePathDelay;
            if (this.updatePathDelay <= 0) {
               this.mob.getNavigation().moveTo(â˜ƒ, this.canRun() ? this.speedModifier : this.speedModifier * 0.5);
         ...
         if (this.crossbowState == RangedCrossbowAttackGoal.CrossbowState.UNCHARGED) {
            ...
            int â˜ƒx = this.mob.getTicksUsingItem();
            ItemStack â˜ƒxx = this.mob.getUseItem();
            if (â˜ƒx >= CrossbowItem.getChargeDuration(â˜ƒxx)) {
         ...
         } else if (this.crossbowState == RangedCrossbowAttackGoal.CrossbowState.READY_TO_ATTACK && â˜ƒx) {
            this.mob.performRangedAttack(â˜ƒ, 1.0F);
            ItemStack â˜ƒx = this.mob.getItemInHand(ProjectileUtil.getWeaponHoldingHand(this.mob, Items.CROSSBOW));
            CrossbowItem.setCharged(â˜ƒx, false);
            this.crossbowState = RangedCrossbowAttackGoal.CrossbowState.UNCHARGED;
         }
      }
   }
