...
public class GoatAi {
   ...
   protected static void initMemories(Goat var0) {
      â˜ƒ.getBrain().setMemory(MemoryModuleType.LONG_JUMP_COOLDOWN_TICKS, TIME_BETWEEN_LONG_JUMPS.sample(â˜ƒ.level.random));
      â˜ƒ.getBrain().setMemory(MemoryModuleType.RAM_COOLDOWN_TICKS, TIME_BETWEEN_RAMS.sample(â˜ƒ.level.random));
   }

   protected static Brain<?> makeBrain(Brain<Goat> var0) {
      initCoreActivity(â˜ƒ);
      initIdleActivity(â˜ƒ);
      initLongJumpActivity(â˜ƒ);
      initRamActivity(â˜ƒ);
   ...
   public static void updateActivity(Goat var0) {
      â˜ƒ.getBrain().setActiveActivityToFirstValid(ImmutableList.of(Activity.RAM, Activity.LONG_JUMP, Activity.IDLE));
   }
