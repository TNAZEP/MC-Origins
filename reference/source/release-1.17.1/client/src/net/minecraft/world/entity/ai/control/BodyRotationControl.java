...
public class BodyRotationControl implements Control {
   ...
   private void rotateHeadTowardsFront() {
      int â˜ƒ = this.headStableTime - 10;
      float â˜ƒx = Mth.clamp((float)â˜ƒ / 10.0F, 0.0F, 1.0F);
      float â˜ƒxx = (float)this.mob.getMaxHeadYRot() * (1.0F - â˜ƒx);
      this.mob.yBodyRot = Mth.rotateIfNecessary(this.mob.yBodyRot, this.mob.yHeadRot, â˜ƒxx);
   }
   ...
   private boolean isMoving() {
      double â˜ƒ = this.mob.getX() - this.mob.xo;
      double â˜ƒx = this.mob.getZ() - this.mob.zo;
      return â˜ƒ * â˜ƒ + â˜ƒx * â˜ƒx > 2.5000003E-7F;
   }
}
