...
public class RangedBowAttackGoal<T extends Monster & RangedAttackMob> extends Goal {
   ...
   public void tick() {
      ...
      if (â˜ƒ != null) {
         ...
         boolean â˜ƒxx = this.mob.getSensing().hasLineOfSight(â˜ƒ);
         boolean â˜ƒxxx = this.seeTime > 0;
         if (â˜ƒxx != â˜ƒxxx) {
            this.seeTime = 0;
         }
         ...
         if (!(â˜ƒx > (double)this.attackRadiusSqr) && this.seeTime >= 20) {
            this.mob.getNavigation().stop();
            ++this.strafingTime;
         } else {
            this.mob.getNavigation().moveTo(â˜ƒ, this.speedModifier);
            this.strafingTime = -1;
         }
         ...
         if (this.strafingTime > -1) {
            if (â˜ƒx > (double)(this.attackRadiusSqr * 0.75F)) {
               this.strafingBackwards = false;
            } else if (â˜ƒx < (double)(this.attackRadiusSqr * 0.25F)) {
               this.strafingBackwards = true;
            }

            this.mob.getMoveControl().strafe(this.strafingBackwards ? -0.5F : 0.5F, this.strafingClockwise ? 0.5F : -0.5F);
            this.mob.lookAt(â˜ƒ, 30.0F, 30.0F);
         } else {
            this.mob.getLookControl().setLookAt(â˜ƒ, 30.0F, 30.0F);
         }

         if (this.mob.isUsingItem()) {
            if (!â˜ƒxx && this.seeTime < -60) {
               this.mob.stopUsingItem();
            } else if (â˜ƒxx) {
               int â˜ƒx = this.mob.getTicksUsingItem();
               if (â˜ƒx >= 20) {
                  this.mob.stopUsingItem();
                  this.mob.performRangedAttack(â˜ƒ, BowItem.getPowerForTime(â˜ƒx));
