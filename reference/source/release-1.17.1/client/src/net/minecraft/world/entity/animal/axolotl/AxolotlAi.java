...
public class AxolotlAi {
   ...
   private static void initPlayDeadActivity(Brain<Axolotl> var0) {
      â˜ƒ.addActivityAndRemoveMemoriesWhenStopped(
         Activity.PLAY_DEAD,
   ...
   private static boolean canSetWalkTargetFromLookTarget(LivingEntity var0) {
      Level â˜ƒ = â˜ƒ.level;
      Optional<PositionTracker> â˜ƒx = â˜ƒ.getBrain().getMemory(MemoryModuleType.LOOK_TARGET);
      if (â˜ƒx.isPresent()) {
         BlockPos â˜ƒxx = ((PositionTracker)â˜ƒx.get()).currentBlockPosition();
         return â˜ƒ.isWaterAt(â˜ƒxx) == â˜ƒ.isInWaterOrBubble();
      } else {
         return false;
      }
   }
   ...
   public static void updateActivity(Axolotl var0) {
      ...
      if (â˜ƒx != Activity.PLAY_DEAD) {
         â˜ƒ.setActiveActivityToFirstValid(ImmutableList.of(Activity.PLAY_DEAD, Activity.FIGHT, Activity.IDLE));
         if (â˜ƒx == Activity.FIGHT && â˜ƒ.getActiveNonCoreActivity().orElse(null) != Activity.FIGHT) {
            â˜ƒ.setMemoryWithExpiry(MemoryModuleType.HAS_HUNTING_COOLDOWN, true, 2400L);
         }
      }
   }

   private static float getSpeedModifierChasing(LivingEntity var0) {
      return â˜ƒ.isInWaterOrBubble() ? 0.6F : 0.15F;
   }
   ...
   private static float getSpeedModifier(LivingEntity var0) {
      return â˜ƒ.isInWaterOrBubble() ? 0.5F : 0.15F;
   }

   private static Optional<? extends LivingEntity> findNearestValidAttackTarget(Axolotl var0) {
      return isBreeding(â˜ƒ) ? Optional.empty() : â˜ƒ.getBrain().getMemory(MemoryModuleType.NEAREST_ATTACKABLE);
   }
