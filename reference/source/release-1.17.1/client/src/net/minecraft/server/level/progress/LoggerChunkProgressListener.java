...
public class LoggerChunkProgressListener implements ChunkProgressListener {
   ...
   public void onStatusChange(ChunkPos var1, @Nullable ChunkStatus var2) {
      if (â˜ƒ == ChunkStatus.FULL) {
         ++this.count;
      }

      int â˜ƒ = this.getProgress();
      if (Util.getMillis() > this.nextTickTime) {
         this.nextTickTime += 500L;
         LOGGER.info(new TranslatableComponent("menu.preparingSpawn", Mth.clamp(â˜ƒ, 0, 100)).getString());
      }
   }
