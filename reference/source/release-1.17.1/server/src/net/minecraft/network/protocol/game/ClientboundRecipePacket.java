...
public class ClientboundRecipePacket implements Packet<ClientGamePacketListener> {
   ...
   public ClientboundRecipePacket(
      ...
      this.toHighlight = ImmutableList.copyOf(â˜ƒ);
      this.bookSettings = â˜ƒ;
   }

   public ClientboundRecipePacket(FriendlyByteBuf var1) {
      this.state = â˜ƒ.readEnum(ClientboundRecipePacket.State.class);
      this.bookSettings = RecipeBookSettings.read(â˜ƒ);
      this.recipes = â˜ƒ.readList(FriendlyByteBuf::readResourceLocation);
      if (this.state == ClientboundRecipePacket.State.INIT) {
         this.toHighlight = â˜ƒ.readList(FriendlyByteBuf::readResourceLocation);
      } else {
   ...
   public void write(FriendlyByteBuf var1) {
      â˜ƒ.writeEnum(this.state);
      this.bookSettings.write(â˜ƒ);
      â˜ƒ.writeCollection(this.recipes, FriendlyByteBuf::writeResourceLocation);
      if (this.state == ClientboundRecipePacket.State.INIT) {
         â˜ƒ.writeCollection(this.toHighlight, FriendlyByteBuf::writeResourceLocation);
      }
   }

   public void handle(ClientGamePacketListener var1) {
      â˜ƒ.handleAddOrRemoveRecipes(this);
   }
