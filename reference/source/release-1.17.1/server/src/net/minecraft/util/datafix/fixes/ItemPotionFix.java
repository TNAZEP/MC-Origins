   ...
   public TypeRewriteRule makeRule() {
      ...
      return this.fixTypeEverywhereTyped(
         ...
         var2x -> {
            ...
            if (â˜ƒ.isPresent() && Objects.equals(((Pair)â˜ƒ.get()).getSecond(), "minecraft:potion")) {
               Dynamic<?> â˜ƒx = var2x.get(DSL.remainderFinder());
               Optional<? extends Typed<?>> â˜ƒxx = var2x.getOptionalTyped(â˜ƒ);
               short â˜ƒxxx = â˜ƒx.get("Damage").asShort((short)0);
               if (â˜ƒxx.isPresent()) {
                  Typed<?> â˜ƒxxxx = var2x;
                  Dynamic<?> â˜ƒxxxxx = ((Typed)â˜ƒxx.get()).get(DSL.remainderFinder());
                  Optional<String> â˜ƒxxxxxx = â˜ƒxxxxx.get("Potion").asString().result();
                  if (!â˜ƒxxxxxx.isPresent()) {
                     String â˜ƒxxxxxxx = POTIONS[â˜ƒxxx & 127];
                     Typed<?> â˜ƒxxxxxxxx = ((Typed)â˜ƒxx.get())
                        .set(DSL.remainderFinder(), â˜ƒxxxxx.set("Potion", â˜ƒxxxxx.createString(â˜ƒxxxxxxx == null ? "minecraft:water" : â˜ƒxxxxxxx)));
                     â˜ƒxxxx = var2x.set(â˜ƒ, â˜ƒxxxxxxxx);
                     if ((â˜ƒxxx & 16384) == 16384) {
                        â˜ƒxxxx = â˜ƒxxxx.set(â˜ƒ, Pair.of(References.ITEM_NAME.typeName(), "minecraft:splash_potion"));
                     }
                  }
                  ...
                  return â˜ƒxxxx.set(DSL.remainderFinder(), â˜ƒx);
               }
            }
