...
public class BlockEntityIdFix extends DataFix {
   ...
   public TypeRewriteRule makeRule() {
      ...
      Type<?> â˜ƒx = this.getOutputSchema().getType(References.ITEM_STACK);
      TaggedChoiceType<String> â˜ƒxx = this.getInputSchema().findChoiceType(References.BLOCK_ENTITY);
      TaggedChoiceType<String> â˜ƒxxx = this.getOutputSchema().findChoiceType(References.BLOCK_ENTITY);
      return TypeRewriteRule.seq(
         this.convertUnchecked("item stack block entity name hook converter", â˜ƒ, â˜ƒx),
         this.fixTypeEverywhere("BlockEntityIdFix", â˜ƒxx, â˜ƒxxx, var0 -> var0x -> var0x.mapFirst(var0xx -> (String)ID_MAP.getOrDefault(var0xx, var0xx)))
      );
   }
}
