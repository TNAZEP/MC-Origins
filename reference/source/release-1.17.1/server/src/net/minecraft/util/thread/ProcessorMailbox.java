...
public class ProcessorMailbox<T> implements ProfilerMeasured, ProcessorHandle<T>, AutoCloseable, Runnable {
   ...
   public static ProcessorMailbox<Runnable> create(Executor var0, String var1) {
      return new ProcessorMailbox<>(new StrictQueue.QueueStrictQueue(new ConcurrentLinkedQueue()), â˜ƒ, â˜ƒ);
   }
   ...
   private boolean setAsScheduled() {
      ...
      } while(!this.status.compareAndSet(â˜ƒ, â˜ƒ | 2));
   ...
   private void setAsIdle() {
      ...
      do {
         â˜ƒ = this.status.get();
      } while(!this.status.compareAndSet(â˜ƒ, â˜ƒ & -3));
   }
   ...
   public void close() {
      ...
      do {
         â˜ƒ = this.status.get();
      } while(!this.status.compareAndSet(â˜ƒ, â˜ƒ | 1));
   }
   ...
   private boolean pollTask() {
      if (!this.shouldProcess()) {
         ...
         Runnable â˜ƒ = (Runnable)this.queue.pop();
         if (â˜ƒ == null) {
            return false;
         } else {
            Util.wrapThreadWithTaskName(this.name, â˜ƒ).run();
            return true;
         }
      }
   }
   ...
   private int pollUntil(Int2BooleanFunction var1) {
      ...
      while(â˜ƒ.get(â˜ƒ) && this.pollTask()) {
         ++â˜ƒ;
      }
