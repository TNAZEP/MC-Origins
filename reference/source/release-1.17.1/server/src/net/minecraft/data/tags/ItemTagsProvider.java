...
public class ItemTagsProvider extends TagsProvider<Item> {
   ...
   public ItemTagsProvider(DataGenerator var1, BlockTagsProvider var2) {
      super(â˜ƒ, Registry.ITEM);
      this.blockTags = â˜ƒ::getOrCreateRawBuilder;
   }
   ...
   protected void copy(Tag.Named<Block> var1, Tag.Named<Item> var2) {
      ...
      Tag.Builder â˜ƒx = (Tag.Builder)this.blockTags.apply(â˜ƒ);
      â˜ƒx.getEntries().forEach(â˜ƒ::add);
   }

   @Override
   protected Path getPath(ResourceLocation var1) {
      return this.generator.getOutputFolder().resolve("data/" + â˜ƒ.getNamespace() + "/tags/items/" + â˜ƒ.getPath() + ".json");
   }
