...
public class BlockLoot implements Consumer<BiConsumer<ResourceLocation, LootTable.Builder>> {
   ...
   private static <T> T applyExplosionDecay(ItemLike var0, FunctionUserBuilder<T> var1) {
      return (T)(!EXPLOSION_RESISTANT.contains(â˜ƒ.asItem()) ? â˜ƒ.apply(ApplyExplosionDecay.explosionDecay()) : â˜ƒ.unwrap());
   }

   private static <T> T applyExplosionCondition(ItemLike var0, ConditionUserBuilder<T> var1) {
      return (T)(!EXPLOSION_RESISTANT.contains(â˜ƒ.asItem()) ? â˜ƒ.when(ExplosionCondition.survivesExplosion()) : â˜ƒ.unwrap());
   }

   private static LootTable.Builder createSingleItemTable(ItemLike var0) {
      return LootTable.lootTable()
         .withPool(applyExplosionCondition(â˜ƒ, LootPool.lootPool().setRolls(ConstantValue.exactly(1.0F)).add(LootItem.lootTableItem(â˜ƒ))));
   }
   ...
   private static LootTable.Builder createSingleItemTable(ItemLike var0, NumberProvider var1) {
      return LootTable.lootTable()
         .withPool(
            LootPool.lootPool()
               .setRolls(ConstantValue.exactly(1.0F))
               .add(applyExplosionDecay(â˜ƒ, LootItem.lootTableItem(â˜ƒ).apply(SetItemCountFunction.setCount(â˜ƒ))))
         );
   }

   private static LootTable.Builder createSingleItemTableWithSilkTouch(Block var0, ItemLike var1, NumberProvider var2) {
      return createSilkTouchDispatchTable(â˜ƒ, applyExplosionDecay(â˜ƒ, LootItem.lootTableItem(â˜ƒ).apply(SetItemCountFunction.setCount(â˜ƒ))));
   }

   private static LootTable.Builder createSilkTouchOnlyTable(ItemLike var0) {
      return LootTable.lootTable().withPool(LootPool.lootPool().when(HAS_SILK_TOUCH).setRolls(ConstantValue.exactly(1.0F)).add(LootItem.lootTableItem(â˜ƒ)));
   }
         ...
         .withPool(
            LootPool.lootPool()
               ...
               .add(
                  applyExplosionDecay(
                     â˜ƒ,
                     LootItem.lootTableItem(â˜ƒ)
                        .apply(
                           SetItemCountFunction.setCount(ConstantValue.exactly(2.0F))
                              .when(
                                 LootItemBlockStatePropertyCondition.hasBlockStateProperties(â˜ƒ)
               ...
               LootPool.lootPool()
                  ...
                  .add(
                     LootItem.lootTableItem(â˜ƒ)
                        .when(
                           LootItemBlockStatePropertyCondition.hasBlockStateProperties(â˜ƒ)
                              .setProperties(StatePropertiesPredicate.Builder.properties().hasProperty(â˜ƒ, â˜ƒ))
                        )
                  )
            )
         );
      ...
      return LootTable.lootTable()
         .withPool(
            applyExplosionCondition(
               ...
               LootPool.lootPool()
                  .setRolls(ConstantValue.exactly(1.0F))
                  .add(LootItem.lootTableItem(â˜ƒ).apply(CopyNameFunction.copyName(CopyNameFunction.NameSource.BLOCK_ENTITY)))
            )
         );
   }
            ...
            LootPool.lootPool()
               ...
               .add(
                  LootItem.lootTableItem(â˜ƒ)
                     ...
                     .apply(CopyBlockState.copyState(â˜ƒ).copy(BeehiveBlock.HONEY_LEVEL))
               )
         );
   }
      ...
      return LootTable.lootTable()
         .withPool(
            LootPool.lootPool()
               .setRolls(ConstantValue.exactly(1.0F))
               .add(
                  ((LootPoolSingletonContainer.Builder)LootItem.lootTableItem(â˜ƒ).when(HAS_SILK_TOUCH))
                     .apply(CopyNbtFunction.copyData(ContextNbtProvider.BLOCK_ENTITY).copy("Bees", "BlockEntityTag.Bees"))
                     .apply(CopyBlockState.copyState(â˜ƒ).copy(BeehiveBlock.HONEY_LEVEL))
                     .otherwise(LootItem.lootTableItem(â˜ƒ))
               )
         );
   }
   ...
   private static LootTable.Builder createOreDrop(Block var0, Item var1) {
      return createSilkTouchDispatchTable(
         â˜ƒ, applyExplosionDecay(â˜ƒ, LootItem.lootTableItem(â˜ƒ).apply(ApplyBonusCount.addOreBonusCount(Enchantments.BLOCK_FORTUNE)))
      );
   }
      ...
      return LootTable.lootTable()
         .withPool(
            applyExplosionDecay(
               ...
               LootPool.lootPool()
                  .setRolls(ConstantValue.exactly(1.0F))
                  .add(LootItem.lootTableItem(â˜ƒ).apply(SetItemCountFunction.setCount(BinomialDistributionGenerator.binomial(3, 0.53333336F))))
            )
         );
   }

   private static LootTable.Builder createShearsOnlyDrop(ItemLike var0) {
      return LootTable.lootTable().withPool(LootPool.lootPool().setRolls(ConstantValue.exactly(1.0F)).when(HAS_SHEARS).add(LootItem.lootTableItem(â˜ƒ)));
   }
         ...
         .withPool(
            LootPool.lootPool()
               .add(
                  applyExplosionDecay(
                     â˜ƒ,
                     ((LootPoolSingletonContainer.Builder)LootItem.lootTableItem(â˜ƒ).when(HAS_SHEARS))
                        .apply(
   ...
   private static LootTable.Builder createLeavesDrops(Block var0, Block var1, float... var2) {
      return createSilkTouchOrShearsDispatchTable(
            â˜ƒ, applyExplosionCondition(â˜ƒ, LootItem.lootTableItem(â˜ƒ)).when(BonusLevelTableCondition.bonusLevelFlatChance(Enchantments.BLOCK_FORTUNE, â˜ƒ))
         )
         .withPool(
            LootPool.lootPool()
               .setRolls(ConstantValue.exactly(1.0F))
               ...
               .add(
                  applyExplosionDecay(â˜ƒ, LootItem.lootTableItem(Items.STICK).apply(SetItemCountFunction.setCount(UniformGenerator.between(1.0F, 2.0F))))
   ...
   private static LootTable.Builder createOakLeavesDrops(Block var0, Block var1, float... var2) {
      return createLeavesDrops(â˜ƒ, â˜ƒ, â˜ƒ)
         .withPool(
            LootPool.lootPool()
               .setRolls(ConstantValue.exactly(1.0F))
               .when(HAS_NO_SHEARS_OR_SILK_TOUCH)
               .add(
                  applyExplosionCondition(â˜ƒ, LootItem.lootTableItem(Items.APPLE))
   ...
   private static LootTable.Builder createCropDrops(Block var0, Item var1, Item var2, LootItemCondition.Builder var3) {
      return applyExplosionDecay(
         ...
         LootTable.lootTable()
            .withPool(
               LootPool.lootPool().add(((LootPoolSingletonContainer.Builder)LootItem.lootTableItem(â˜ƒ).when(â˜ƒ)).otherwise(LootItem.lootTableItem(â˜ƒ)))
            )
            .withPool(
               LootPool.lootPool()
                  .when(â˜ƒ)
                  .add(LootItem.lootTableItem(â˜ƒ).apply(ApplyBonusCount.addBonusBinomialDistributionCount(Enchantments.BLOCK_FORTUNE, 0.5714286F, 3)))
            )
      );
   }
   ...
   private static LootTable.Builder createDoublePlantShearsDrop(Block var0) {
      return LootTable.lootTable()
         .withPool(LootPool.lootPool().when(HAS_SHEARS).add(LootItem.lootTableItem(â˜ƒ).apply(SetItemCountFunction.setCount(ConstantValue.exactly(2.0F)))));
   }

   private static LootTable.Builder createDoublePlantWithSeedDrops(Block var0, Block var1) {
      LootPoolEntryContainer.Builder<?> â˜ƒ = ((LootPoolSingletonContainer.Builder)LootItem.lootTableItem(â˜ƒ)
            .apply(SetItemCountFunction.setCount(ConstantValue.exactly(2.0F)))
            .when(HAS_SHEARS))
         .otherwise(applyExplosionCondition(â˜ƒ, LootItem.lootTableItem(Items.WHEAT_SEEDS)).when(LootItemRandomChanceCondition.randomChance(0.125F)));
      return LootTable.lootTable()
         .withPool(
            LootPool.lootPool()
               .add(â˜ƒ)
   ...
   private static LootTable.Builder createCandleCakeDrops(Block var0) {
      return LootTable.lootTable().withPool(LootPool.lootPool().setRolls(ConstantValue.exactly(1.0F)).add(LootItem.lootTableItem(â˜ƒ)));
   }
   ...
   public void accept(BiConsumer<ResourceLocation, LootTable.Builder> var1) {
      ...
      LootItemCondition.Builder â˜ƒ = LootItemBlockStatePropertyCondition.hasBlockStateProperties(Blocks.BEETROOTS)
         .setProperties(StatePropertiesPredicate.Builder.properties().hasProperty(BeetrootBlock.AGE, 3));
      this.add(Blocks.BEETROOTS, createCropDrops(Blocks.BEETROOTS, Items.BEETROOT, Items.BEETROOT_SEEDS, â˜ƒ));
      LootItemCondition.Builder â˜ƒx = LootItemBlockStatePropertyCondition.hasBlockStateProperties(Blocks.WHEAT)
         .setProperties(StatePropertiesPredicate.Builder.properties().hasProperty(CropBlock.AGE, 7));
      this.add(Blocks.WHEAT, createCropDrops(Blocks.WHEAT, Items.WHEAT, Items.WHEAT_SEEDS, â˜ƒx));
      LootItemCondition.Builder â˜ƒxx = LootItemBlockStatePropertyCondition.hasBlockStateProperties(Blocks.CARROTS)
      ...
      LootItemCondition.Builder â˜ƒxxx = LootItemBlockStatePropertyCondition.hasBlockStateProperties(Blocks.POTATOES)
      ...
      this.add(
         ...
         applyExplosionDecay(
            ...
            LootTable.lootTable()
               ...
               .withPool(
                  LootPool.lootPool().when(â˜ƒxxx).add(LootItem.lootTableItem(Items.POISONOUS_POTATO).when(LootItemRandomChanceCondition.randomChance(0.02F)))
               )
         )
      );
      this.add(
      ...
      for(Block â˜ƒxxxxx : Registry.BLOCK) {
         ResourceLocation â˜ƒxxxxxx = â˜ƒxxxxx.getLootTable();
         if (â˜ƒxxxxxx != BuiltInLootTables.EMPTY && â˜ƒxxxx.add(â˜ƒxxxxxx)) {
            LootTable.Builder â˜ƒxxxxxxx = (LootTable.Builder)this.map.remove(â˜ƒxxxxxx);
            if (â˜ƒxxxxxxx == null) {
               throw new IllegalStateException(String.format("Missing loottable '%s' for '%s'", â˜ƒxxxxxx, Registry.BLOCK.getKey(â˜ƒxxxxx)));
            }

            â˜ƒ.accept(â˜ƒxxxxxx, â˜ƒxxxxxxx);
         }
      }
   ...
   private void addNetherVinesDropTable(Block var1, Block var2) {
      LootTable.Builder â˜ƒ = createSilkTouchOrShearsDispatchTable(
         â˜ƒ, LootItem.lootTableItem(â˜ƒ).when(BonusLevelTableCondition.bonusLevelFlatChance(Enchantments.BLOCK_FORTUNE, 0.33F, 0.55F, 0.77F, 1.0F))
      );
      this.add(â˜ƒ, â˜ƒ);
      this.add(â˜ƒ, â˜ƒ);
   }
   ...
   public static LootTable.Builder createDoorTable(Block var0) {
      return createSinglePropConditionTable(â˜ƒ, DoorBlock.HALF, DoubleBlockHalf.LOWER);
   }

   public void dropPottedContents(Block var1) {
      this.add(â˜ƒ, var0 -> createPotFlowerItemTable(((FlowerPotBlock)var0).getContent()));
   }

   public void otherWhenSilkTouch(Block var1, Block var2) {
      this.add(â˜ƒ, createSilkTouchOnlyTable(â˜ƒ));
   }

   public void dropOther(Block var1, ItemLike var2) {
      this.add(â˜ƒ, createSingleItemTable(â˜ƒ));
   }

   public void dropWhenSilkTouch(Block var1) {
      this.otherWhenSilkTouch(â˜ƒ, â˜ƒ);
   }

   public void dropSelf(Block var1) {
      this.dropOther(â˜ƒ, â˜ƒ);
   }

   private void add(Block var1, Function<Block, LootTable.Builder> var2) {
      this.add(â˜ƒ, (LootTable.Builder)â˜ƒ.apply(â˜ƒ));
   }

   private void add(Block var1, LootTable.Builder var2) {
      this.map.put(â˜ƒ.getLootTable(), â˜ƒ);
   }
}
