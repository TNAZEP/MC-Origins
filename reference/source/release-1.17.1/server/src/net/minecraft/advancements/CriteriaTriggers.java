...
public class CriteriaTriggers {
   ...
   private static <T extends CriterionTrigger<?>> T register(T var0) {
      if (CRITERIA.containsKey(â˜ƒ.getId())) {
         throw new IllegalArgumentException("Duplicate criterion id " + â˜ƒ.getId());
      } else {
         CRITERIA.put(â˜ƒ.getId(), â˜ƒ);
         return â˜ƒ;
      }
   }
   ...
   public static <T extends CriterionTriggerInstance> CriterionTrigger<T> getCriterion(ResourceLocation var0) {
      return (CriterionTrigger<T>)CRITERIA.get(â˜ƒ);
   }
