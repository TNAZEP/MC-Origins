...
public abstract class Registry<T> implements Codec<T>, Keyable, IdMap<T> {
   ...
   private static <T> ResourceKey<Registry<T>> createRegistryKey(String var0) {
      return ResourceKey.createRegistryKey(new ResourceLocation(â˜ƒ));
   }

   public static <T extends WritableRegistry<?>> void checkRegistry(WritableRegistry<T> var0) {
      â˜ƒ.forEach(var1 -> {
         if (var1.keySet().isEmpty()) {
            Util.logAndPauseIfInIde("Registry '" + â˜ƒ.getKey((T)var1) + "' was empty after loading");
         }

         if (var1 instanceof DefaultedRegistry) {
            ResourceLocation â˜ƒ = ((DefaultedRegistry)var1).getDefaultKey();
            Validate.notNull(var1.get(â˜ƒ), "Missing default of DefaultedMappedRegistry: " + â˜ƒ);
         }
      });
   }

   private static <T> Registry<T> registerSimple(ResourceKey<? extends Registry<T>> var0, Supplier<T> var1) {
      return registerSimple(â˜ƒ, Lifecycle.experimental(), â˜ƒ);
   }

   private static <T> DefaultedRegistry<T> registerDefaulted(ResourceKey<? extends Registry<T>> var0, String var1, Supplier<T> var2) {
      return registerDefaulted(â˜ƒ, â˜ƒ, Lifecycle.experimental(), â˜ƒ);
   }
   ...
   private static <T> DefaultedRegistry<T> registerDefaulted(ResourceKey<? extends Registry<T>> var0, String var1, Lifecycle var2, Supplier<T> var3) {
      return internalRegister(â˜ƒ, new DefaultedRegistry<>(â˜ƒ, â˜ƒ, â˜ƒ), â˜ƒ, â˜ƒ);
   }
   ...
   protected Registry(ResourceKey<? extends Registry<T>> var1, Lifecycle var2) {
      Bootstrap.checkBootstrapCalled(() -> "registry " + â˜ƒ);
      this.key = â˜ƒ;
      this.lifecycle = â˜ƒ;
   }
   ...
   public <U> DataResult<Pair<T, U>> decode(DynamicOps<U> var1, U var2) {
      return â˜ƒ.compressMaps()
         ? â˜ƒ.getNumberValue(â˜ƒ).flatMap(var1x -> {
            T â˜ƒ = this.byId(var1x.intValue());
            return â˜ƒ == null ? DataResult.error("Unknown registry id: " + var1x) : DataResult.success(â˜ƒ, this.lifecycle(â˜ƒ));
         }).map(var1x -> Pair.of(var1x, â˜ƒ.empty()))
         : ResourceLocation.CODEC
            .decode(â˜ƒ, â˜ƒ)
            .flatMap(
               var1x -> {
                  T â˜ƒ = this.get((ResourceLocation)var1x.getFirst());
                  return â˜ƒ == null
                     ? DataResult.error("Unknown registry key: " + var1x.getFirst())
                     : DataResult.success(Pair.of(â˜ƒ, var1x.getSecond()), this.lifecycle(â˜ƒ));
               }
            );
   }
   ...
   public <U> DataResult<U> encode(T var1, DynamicOps<U> var2, U var3) {
      ResourceLocation â˜ƒ = this.getKey(â˜ƒ);
      if (â˜ƒ == null) {
         return DataResult.error("Unknown registry element " + â˜ƒ);
      } else {
         return â˜ƒ.compressMaps()
            ? â˜ƒ.mergeToPrimitive(â˜ƒ, â˜ƒ.createInt(this.getId(â˜ƒ))).setLifecycle(this.lifecycle)
            : â˜ƒ.mergeToPrimitive(â˜ƒ, â˜ƒ.createString(â˜ƒ.toString())).setLifecycle(this.lifecycle);
      }
   }

   @Override
   public <U> Stream<U> keys(DynamicOps<U> var1) {
      return this.keySet().stream().map(var1x -> â˜ƒ.createString(var1x.toString()));
   }
   ...
   public Optional<T> getOptional(@Nullable ResourceLocation var1) {
      return Optional.ofNullable(this.get(â˜ƒ));
   }
   ...
   public T getOrThrow(ResourceKey<T> var1) {
      T â˜ƒ = this.get(â˜ƒ);
      if (â˜ƒ == null) {
         throw new IllegalStateException("Missing: " + â˜ƒ);
      } else {
         return â˜ƒ;
      }
   }
   ...
   public static <V, T extends V> T registerMapping(Registry<V> var0, int var1, String var2, T var3) {
      return ((WritableRegistry)â˜ƒ).registerMapping(â˜ƒ, ResourceKey.create(â˜ƒ.key, new ResourceLocation(â˜ƒ)), â˜ƒ, Lifecycle.stable());
   }
