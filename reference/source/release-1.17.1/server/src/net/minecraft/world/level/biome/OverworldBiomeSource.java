...
public class OverworldBiomeSource extends BiomeSource {
   public static final Codec<OverworldBiomeSource> CODEC = RecordCodecBuilder.create(
      var0 -> var0.group(
               ...
               Codec.BOOL.optionalFieldOf("legacy_biome_init_layer", Boolean.valueOf(false), Lifecycle.stable()).forGetter(var0x -> var0x.legacyBiomeInitLayer),
               Codec.BOOL.fieldOf("large_biomes").orElse(false).stable().forGetter(var0x -> var0x.largeBiomes),
   ...
   public OverworldBiomeSource(long var1, boolean var3, boolean var4, Registry<Biome> var5) {
      super(POSSIBLE_BIOMES.stream().map(var1x -> () -> â˜ƒ.getOrThrow(var1x)));
      this.seed = â˜ƒ;
      this.legacyBiomeInitLayer = â˜ƒ;
      this.largeBiomes = â˜ƒ;
      this.biomes = â˜ƒ;
      this.noiseBiomeLayer = Layers.getDefaultLayer(â˜ƒ, â˜ƒ, â˜ƒ ? 6 : 4, 4);
   }
   ...
   public BiomeSource withSeed(long var1) {
      return new OverworldBiomeSource(â˜ƒ, this.legacyBiomeInitLayer, this.largeBiomes, this.biomes);
   }

   @Override
   public Biome getNoiseBiome(int var1, int var2, int var3) {
      return this.noiseBiomeLayer.get(this.biomes, â˜ƒ, â˜ƒ);
   }
}
