...
public class Fluids {
   ...
   private static <T extends Fluid> T register(String var0, T var1) {
      return Registry.register(Registry.FLUID, â˜ƒ, â˜ƒ);
   }

   static {
      for(Fluid â˜ƒ : Registry.FLUID) {
         for(FluidState â˜ƒx : â˜ƒ.getStateDefinition().getPossibleStates()) {
            Fluid.FLUID_STATE_REGISTRY.add(â˜ƒx);
         }
      }
   }
}
