...
public class BlockEntityType<T extends BlockEntity> {
   ...
   public static ResourceLocation getKey(BlockEntityType<?> var0) {
      return Registry.BLOCK_ENTITY_TYPE.getKey(â˜ƒ);
   }

   private static <T extends BlockEntity> BlockEntityType<T> register(String var0, BlockEntityType.Builder<T> var1) {
      if (â˜ƒ.validBlocks.isEmpty()) {
         LOGGER.warn("Block entity type {} requires at least one valid block to be defined!", â˜ƒ);
      }

      Type<?> â˜ƒ = Util.fetchChoiceType(References.BLOCK_ENTITY, â˜ƒ);
      return Registry.register(Registry.BLOCK_ENTITY_TYPE, â˜ƒ, â˜ƒ.build(â˜ƒ));
   }
   ...
   public static final class Builder<T extends BlockEntity> {
      ...
      public static <T extends BlockEntity> BlockEntityType.Builder<T> of(BlockEntityType.BlockEntitySupplier<? extends T> var0, Block... var1) {
         return new BlockEntityType.Builder<>(â˜ƒ, ImmutableSet.copyOf(â˜ƒ));
      }

      public BlockEntityType<T> build(Type<?> var1) {
         return new BlockEntityType<>(this.factory, this.validBlocks, â˜ƒ);
      }
   }
}
