...
public class ObjectiveCriteria {
   ...
   private static ObjectiveCriteria registerCustom(String var0, boolean var1, ObjectiveCriteria.RenderType var2) {
      ObjectiveCriteria â˜ƒ = new ObjectiveCriteria(â˜ƒ, â˜ƒ, â˜ƒ);
      CUSTOM_CRITERIA.put(â˜ƒ, â˜ƒ);
      return â˜ƒ;
   }

   private static ObjectiveCriteria registerCustom(String var0) {
      return registerCustom(â˜ƒ, false, ObjectiveCriteria.RenderType.INTEGER);
   }

   protected ObjectiveCriteria(String var1) {
      this(â˜ƒ, false, ObjectiveCriteria.RenderType.INTEGER);
   }
   ...
   public static Optional<ObjectiveCriteria> byName(String var0) {
      ObjectiveCriteria â˜ƒ = (ObjectiveCriteria)CRITERIA_CACHE.get(â˜ƒ);
      if (â˜ƒ != null) {
         ...
         return â˜ƒ < 0
            ? Optional.empty()
            : Registry.STAT_TYPE
               .getOptional(ResourceLocation.of(â˜ƒ.substring(0, â˜ƒ), '.'))
               .flatMap(var2x -> getStat(var2x, ResourceLocation.of(â˜ƒ.substring(â˜ƒ + 1), '.')));
      }
   }

   private static <T> Optional<ObjectiveCriteria> getStat(StatType<T> var0, ResourceLocation var1) {
      return â˜ƒ.getRegistry().getOptional(â˜ƒ).map(â˜ƒ::get);
   }
   ...
   public static enum RenderType {
      ...
      public static ObjectiveCriteria.RenderType byId(String var0) {
         return (ObjectiveCriteria.RenderType)BY_ID.getOrDefault(â˜ƒ, INTEGER);
      }

      static {
         Builder<String, ObjectiveCriteria.RenderType> â˜ƒ = ImmutableMap.builder();

         for(ObjectiveCriteria.RenderType â˜ƒx : values()) {
            â˜ƒ.put(â˜ƒx.id, â˜ƒx);
         }
