...
public class SimpleBlockConfiguration implements FeatureConfiguration {
   public static final Codec<SimpleBlockConfiguration> CODEC = RecordCodecBuilder.create(
      var0 -> var0.group(
               BlockStateProvider.CODEC.fieldOf("to_place").forGetter(var0x -> var0x.toPlace),
               BlockState.CODEC.listOf().fieldOf("place_on").orElse(ImmutableList.of()).forGetter(var0x -> var0x.placeOn),
               BlockState.CODEC.listOf().fieldOf("place_in").orElse(ImmutableList.of()).forGetter(var0x -> var0x.placeIn),
               BlockState.CODEC.listOf().fieldOf("place_under").orElse(ImmutableList.of()).forGetter(var0x -> var0x.placeUnder)
            )
   ...
   public SimpleBlockConfiguration(BlockStateProvider var1) {
      this(â˜ƒ, ImmutableList.of(), ImmutableList.of(), ImmutableList.of());
   }
}
