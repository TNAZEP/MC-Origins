...
public class LootContextParamSets {
   ...
   private static LootContextParamSet register(String var0, Consumer<LootContextParamSet.Builder> var1) {
      LootContextParamSet.Builder â˜ƒ = new LootContextParamSet.Builder();
      â˜ƒ.accept(â˜ƒ);
      LootContextParamSet â˜ƒx = â˜ƒ.build();
      ResourceLocation â˜ƒxx = new ResourceLocation(â˜ƒ);
      LootContextParamSet â˜ƒxxx = REGISTRY.put(â˜ƒxx, â˜ƒx);
      if (â˜ƒxxx != null) {
         throw new IllegalStateException("Loot table parameter set " + â˜ƒxx + " is already registered");
      } else {
         return â˜ƒx;
      }
   }
   ...
   public static LootContextParamSet get(ResourceLocation var0) {
      return (LootContextParamSet)REGISTRY.get(â˜ƒ);
   }

   @Nullable
   public static ResourceLocation getKey(LootContextParamSet var0) {
      return (ResourceLocation)REGISTRY.inverse().get(â˜ƒ);
   }
}
