...
public class TraderLlama extends Llama {
   ...
   protected void doPlayerRide(Player var1) {
      ...
      if (!(â˜ƒ instanceof WanderingTrader)) {
         super.doPlayerRide(â˜ƒ);
      }
   }
   ...
   protected static class TraderLlamaDefendWanderingTraderGoal extends TargetGoal {
      ...
      public boolean canUse() {
         if (!this.llama.isLeashed()) {
            ...
            Entity â˜ƒ = this.llama.getLeashHolder();
            if (!(â˜ƒ instanceof WanderingTrader)) {
               return false;
            } else {
               WanderingTrader â˜ƒ = (WanderingTrader)â˜ƒ;
               this.ownerLastHurtBy = â˜ƒ.getLastHurtByMob();
               int â˜ƒx = â˜ƒ.getLastHurtByMobTimestamp();
               return â˜ƒx != this.timestamp && this.canAttack(this.ownerLastHurtBy, TargetingConditions.DEFAULT);
            }
         }
      }
      ...
      public void start() {
         ...
         Entity â˜ƒ = this.llama.getLeashHolder();
         if (â˜ƒ instanceof WanderingTrader) {
            this.timestamp = ((WanderingTrader)â˜ƒ).getLastHurtByMobTimestamp();
         }
