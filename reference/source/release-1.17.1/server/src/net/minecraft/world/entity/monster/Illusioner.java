...
public class Illusioner extends SpellcasterIllager implements RangedAttackMob {
   ...
   public Illusioner(EntityType<? extends Illusioner> var1, Level var2) {
      ...
      for(int â˜ƒ = 0; â˜ƒ < 4; ++â˜ƒ) {
         this.clientSideIllusionOffsets[0][â˜ƒ] = Vec3.ZERO;
         this.clientSideIllusionOffsets[1][â˜ƒ] = Vec3.ZERO;
      }
   }
      ...
      if (this.level.isClientSide && this.isInvisible()) {
         ...
         if (this.hurtTime == 1 || this.tickCount % 1200 == 0) {
            ...
            for(int â˜ƒxx = 0; â˜ƒxx < 4; ++â˜ƒxx) {
               this.clientSideIllusionOffsets[0][â˜ƒxx] = this.clientSideIllusionOffsets[1][â˜ƒxx];
               this.clientSideIllusionOffsets[1][â˜ƒxx] = new Vec3(
            ...
            for(int â˜ƒ = 0; â˜ƒ < 4; ++â˜ƒ) {
               this.clientSideIllusionOffsets[0][â˜ƒ] = this.clientSideIllusionOffsets[1][â˜ƒ];
               this.clientSideIllusionOffsets[1][â˜ƒ] = new Vec3(0.0, 0.0, 0.0);
            }
         }
      }
   }
   ...
   public Vec3[] getIllusionOffsets(float var1) {
      if (this.clientSideIllusionTicks <= 0) {
         ...
         double â˜ƒ = (double)(((float)this.clientSideIllusionTicks - â˜ƒ) / 3.0F);
         â˜ƒ = Math.pow(â˜ƒ, 0.25);
         Vec3[] â˜ƒx = new Vec3[4];

         for(int â˜ƒxx = 0; â˜ƒxx < 4; ++â˜ƒxx) {
            â˜ƒx[â˜ƒxx] = this.clientSideIllusionOffsets[1][â˜ƒxx].scale(1.0 - â˜ƒ).add(this.clientSideIllusionOffsets[0][â˜ƒxx].scale(â˜ƒ));
         }
   ...
   public void performRangedAttack(LivingEntity var1, float var2) {
      ...
      AbstractArrow â˜ƒx = ProjectileUtil.getMobArrow(this, â˜ƒ, â˜ƒ);
