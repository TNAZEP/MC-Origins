package net.minecraft.entity.ai;

import com.google.common.collect.Lists;
import java.util.List;
import net.minecraft.entity.EntityCreature;
import net.minecraft.pathfinding.Path;
import net.minecraft.pathfinding.PathNavigateGround;
import net.minecraft.util.math.BlockPos;
import net.minecraft.util.math.MathHelper;
import net.minecraft.util.math.Vec3d;
import net.minecraft.village.Village;
import net.minecraft.village.VillageDoorInfo;

public class EntityAIMoveThroughVillage extends EntityAIBase {
   private final EntityCreature field_75420_a;
   private final double field_75418_b;
   private Path field_75419_c;
   private VillageDoorInfo field_75416_d;
   private final boolean field_75417_e;
   private final List<VillageDoorInfo> field_75415_f = Lists.<VillageDoorInfo>newArrayList();

   public EntityAIMoveThroughVillage(EntityCreature var1, double var2, boolean var4) {
      this.field_75420_a = ☃;
      this.field_75418_b = ☃;
      this.field_75417_e = ☃;
      this.func_75248_a(1);
      if (!(☃.func_70661_as() instanceof PathNavigateGround)) {
         throw new IllegalArgumentException("Unsupported mob for MoveThroughVillageGoal");
      }
   }

   @Override
   public boolean func_75250_a() {
      this.func_75414_f();
      if (this.field_75417_e && this.field_75420_a.field_70170_p.func_72935_r()) {
         return false;
      } else {
         Village ☃ = this.field_75420_a.field_70170_p.func_175714_ae().func_176056_a(new BlockPos(this.field_75420_a), 0);
         if (☃ == null) {
            return false;
         } else {
            this.field_75416_d = this.func_75412_a(☃);
            if (this.field_75416_d == null) {
               return false;
            } else {
               PathNavigateGround ☃ = (PathNavigateGround)this.field_75420_a.func_70661_as();
               boolean ☃x = ☃.func_179686_g();
               ☃.func_179688_b(false);
               this.field_75419_c = ☃.func_179680_a(this.field_75416_d.func_179852_d());
               ☃.func_179688_b(☃x);
               if (this.field_75419_c != null) {
                  return true;
               } else {
                  Vec3d ☃ = RandomPositionGenerator.func_75464_a(
                     this.field_75420_a,
                     10,
                     7,
                     new Vec3d(
                        (double)this.field_75416_d.func_179852_d().func_177958_n(),
                        (double)this.field_75416_d.func_179852_d().func_177956_o(),
                        (double)this.field_75416_d.func_179852_d().func_177952_p()
                     )
                  );
                  if (☃ == null) {
                     return false;
                  } else {
                     ☃.func_179688_b(false);
                     this.field_75419_c = this.field_75420_a.func_70661_as().func_75488_a(☃.field_72450_a, ☃.field_72448_b, ☃.field_72449_c);
                     ☃.func_179688_b(☃x);
                     return this.field_75419_c != null;
                  }
               }
            }
         }
      }
   }

   @Override
   public boolean func_75253_b() {
      if (this.field_75420_a.func_70661_as().func_75500_f()) {
         return false;
      } else {
         float ☃ = this.field_75420_a.field_70130_N + 4.0F;
         return this.field_75420_a.func_174818_b(this.field_75416_d.func_179852_d()) > (double)(☃ * ☃);
      }
   }

   @Override
   public void func_75249_e() {
      this.field_75420_a.func_70661_as().func_75484_a(this.field_75419_c, this.field_75418_b);
   }

   @Override
   public void func_75251_c() {
      if (this.field_75420_a.func_70661_as().func_75500_f() || this.field_75420_a.func_174818_b(this.field_75416_d.func_179852_d()) < 16.0) {
         this.field_75415_f.add(this.field_75416_d);
      }
   }

   private VillageDoorInfo func_75412_a(Village var1) {
      VillageDoorInfo ☃ = null;
      int ☃x = Integer.MAX_VALUE;

      for(VillageDoorInfo ☃xx : ☃.func_75558_f()) {
         int ☃xxx = ☃xx.func_75474_b(
            MathHelper.func_76128_c(this.field_75420_a.field_70165_t),
            MathHelper.func_76128_c(this.field_75420_a.field_70163_u),
            MathHelper.func_76128_c(this.field_75420_a.field_70161_v)
         );
         if (☃xxx < ☃x && !this.func_75413_a(☃xx)) {
            ☃ = ☃xx;
            ☃x = ☃xxx;
         }
      }

      return ☃;
   }

   private boolean func_75413_a(VillageDoorInfo var1) {
      for(VillageDoorInfo ☃ : this.field_75415_f) {
         if (☃.func_179852_d().equals(☃.func_179852_d())) {
            return true;
         }
      }

      return false;
   }

   private void func_75414_f() {
      if (this.field_75415_f.size() > 15) {
         this.field_75415_f.remove(0);
      }
   }
}
