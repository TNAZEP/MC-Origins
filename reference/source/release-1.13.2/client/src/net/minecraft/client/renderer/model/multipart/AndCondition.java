...
public class AndCondition implements ICondition {
   ...
   public Predicate<IBlockState> getPredicate(StateContainer<Block, IBlockState> var1) {
      List<Predicate<IBlockState>> ☃ = (List)Streams.stream(this.field_188121_c).map(var1x -> var1x.getPredicate(☃)).collect(Collectors.toList());
      return var1x -> ☃.stream().allMatch(var1xx -> var1xx.test(var1x));
   }
}
