package net.minecraft.network.play.server;

import java.io.IOException;
import net.minecraft.entity.Entity;
import net.minecraft.network.Packet;
import net.minecraft.network.PacketBuffer;
import net.minecraft.network.play.INetHandlerPlayClient;

public class SPacketAnimation implements Packet<INetHandlerPlayClient> {
   private int field_148981_a;
   private int field_148980_b;

   public SPacketAnimation() {
   }

   public SPacketAnimation(Entity var1, int var2) {
      this.field_148981_a = ☃.func_145782_y();
      this.field_148980_b = ☃;
   }

   @Override
   public void func_148837_a(PacketBuffer var1) throws IOException {
      this.field_148981_a = ☃.func_150792_a();
      this.field_148980_b = ☃.readUnsignedByte();
   }

   @Override
   public void func_148840_b(PacketBuffer var1) throws IOException {
      ☃.func_150787_b(this.field_148981_a);
      ☃.writeByte(this.field_148980_b);
   }

   public void func_148833_a(INetHandlerPlayClient var1) {
      ☃.func_147279_a(this);
   }
}
