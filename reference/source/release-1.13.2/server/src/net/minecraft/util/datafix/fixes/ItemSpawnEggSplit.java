...
public class ItemSpawnEggSplit extends DataFix {
   private static final Map<String, String> field_199421_a = DataFixUtils.make(Maps.newHashMap(), var0 -> {
   ...
   public TypeRewriteRule makeRule() {
      ...
      OpticFinder<String> ☃xx = DSL.fieldFinder("id", DSL.namespacedString());
      OpticFinder<?> ☃xxx = ☃.findField("tag");
      OpticFinder<?> ☃xxxx = ☃xxx.type().findField("EntityTag");
      return this.fixTypeEverywhereTyped("ItemInstanceSpawnEggFix", ☃, var4x -> {
         Optional<Pair<String, String>> ☃ = var4x.getOptional(☃);
         if (☃.isPresent() && Objects.equals(((Pair)☃.get()).getSecond(), "minecraft:spawn_egg")) {
            Typed<?> ☃x = var4x.getOrCreateTyped(☃);
            Typed<?> ☃xx = ☃x.getOrCreateTyped(☃);
            Optional<String> ☃xxx = ☃xx.getOptional(☃);
            if (☃xxx.isPresent()) {
               return var4x.set(☃, Pair.of(TypeReferences.field_211301_q.typeName(), field_199421_a.getOrDefault(☃xxx.get(), "minecraft:pig_spawn_egg")));
            }
         }
